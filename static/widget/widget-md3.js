...
(function () {
  ...
  var CSS = `
    /* ═══════════════════════════════════════════════════════════════
       ...
       Rebuilt from scratch: tonal palettes, elevation levels, state
       layers, MD motion (emphasized easing), shape scale, ripples.
       Class names / IDs are identical to the previous UI so all
       API, streaming, memory and event logic keeps working untouched.
       ═══════════════════════════════════════════════════════════════ */
    @import url('https://cdn.jsdelivr.net/gh/rastikerdar/vazirmatn@v33.003/Vazirmatn-font-face.css');

    :host{all:initial;--asw-font:'Vazirmatn','Vazir','IRANSans','Roboto','Segoe UI',ui-sans-serif,system-ui,sans-serif}
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    .asw{font-family:var(--asw-font);font-size:var(--asw-font-size,14px);direction:rtl;line-height:1.7;color:var(--asw-text);-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;letter-spacing:0}

    /* ─── M3 tokens — light scheme (derived from --asw-primary) ─── */
    .asw{
      --asw-primary:#6750a4;--asw-secondary:#625b71;--asw-accent:#7d5260;

      /* tonal surfaces */
      --md-on-primary:#ffffff;
      --md-primary-container:color-mix(in srgb,var(--asw-primary) 18%,#ffffff);
      --md-on-primary-container:color-mix(in srgb,var(--asw-primary) 82%,#000000);
      --md-secondary-container:color-mix(in srgb,var(--asw-secondary) 14%,#ffffff);
      --md-on-secondary-container:color-mix(in srgb,var(--asw-secondary) 80%,#000000);

      --asw-bg:#fef7ff;
      --md-surface:#fef7ff;
      --md-surface-1:color-mix(in srgb,var(--asw-primary) 5%,#fef7ff);
      --md-surface-2:color-mix(in srgb,var(--asw-primary) 8%,#fef7ff);
      --md-surface-3:color-mix(in srgb,var(--asw-primary) 11%,#fef7ff);
      --md-surface-4:color-mix(in srgb,var(--asw-primary) 12%,#fef7ff);
      --md-surface-5:color-mix(in srgb,var(--asw-primary) 14%,#fef7ff);
      --asw-surface:var(--md-surface-1);
      --asw-surface-2:var(--md-surface-2);
      --md-surface-variant:color-mix(in srgb,var(--asw-primary) 9%,#e7e0ec);

      --asw-text:#1d1b20;
      --asw-text-secondary:#49454f;
      --asw-text-muted:#79747e;
      --asw-border:color-mix(in srgb,var(--asw-primary) 12%,#cac4d0);
      --asw-border-strong:#79747e;

      --asw-bot-bg:var(--md-surface-variant);--asw-bot-text:#1d1b20;
      --asw-user-bg:var(--md-primary-container);--asw-user-bg-2:var(--md-primary-container);
      --asw-user-text:var(--md-on-primary-container);
      --asw-input-bg:var(--md-surface-3);--asw-input-focus-bg:var(--md-surface-3);

      --asw-success:#2e6b4f;--asw-error:#b3261e;--asw-warning:#7d5800;
      --md-error-container:color-mix(in srgb,#b3261e 12%,#fef7ff);

      /* M3 elevation */
      --md-e1:0 1px 2px rgba(0,0,0,.30),0 1px 3px 1px rgba(0,0,0,.15);
      --md-e2:0 1px 2px rgba(0,0,0,.30),0 2px 6px 2px rgba(0,0,0,.15);
      --md-e3:0 4px 8px 3px rgba(0,0,0,.15),0 1px 3px rgba(0,0,0,.30);
      --md-e4:0 6px 10px 4px rgba(0,0,0,.15),0 2px 3px rgba(0,0,0,.30);
      --md-e5:0 8px 12px 6px rgba(0,0,0,.15),0 4px 4px rgba(0,0,0,.30);
      --asw-shadow:var(--md-e3);

      /* M3 shape scale */
      --asw-radius:28px;--asw-radius-sm:16px;--asw-radius-xs:12px;
      --md-shape-xs:4px;--md-shape-s:8px;--md-shape-m:12px;--md-shape-l:16px;--md-shape-xl:28px;--md-shape-full:999px;

      --asw-panel-w:392px;--asw-panel-h:640px;
      --asw-header-bg:var(--md-surface-2);

      /* M3 motion */
      --md-emph:cubic-bezier(.2,0,0,1);
      --md-emph-dec:cubic-bezier(.05,.7,.1,1);
      --md-emph-acc:cubic-bezier(.3,0,.8,.15);
      --md-standard:cubic-bezier(.2,0,0,1);
      --asw-spring:cubic-bezier(.05,.7,.1,1);--asw-out:cubic-bezier(.2,0,0,1);
      --md-dur-short:150ms;--md-dur-med:300ms;--md-dur-long:500ms;
    }

    /* ─── M3 tokens — dark scheme ─── */
    .asw.dark{
      --md-on-primary:#ffffff;
      --md-primary-container:color-mix(in srgb,var(--asw-primary) 46%,#1d1b20);
      --md-on-primary-container:color-mix(in srgb,var(--asw-primary) 18%,#ffffff);
      --md-secondary-container:color-mix(in srgb,var(--asw-secondary) 40%,#1d1b20);
      --md-on-secondary-container:color-mix(in srgb,var(--asw-secondary) 20%,#ffffff);

      --asw-bg:#141218;
      --md-surface:#141218;
      --md-surface-1:color-mix(in srgb,var(--asw-primary) 6%,#1d1b20);
      --md-surface-2:color-mix(in srgb,var(--asw-primary) 9%,#211f26);
      --md-surface-3:color-mix(in srgb,var(--asw-primary) 12%,#2b2930);
      --md-surface-4:color-mix(in srgb,var(--asw-primary) 13%,#2b2930);
      --md-surface-5:color-mix(in srgb,var(--asw-primary) 15%,#36343b);
      --asw-surface:var(--md-surface-1);
      --asw-surface-2:var(--md-surface-2);
      --md-surface-variant:#49454f;

      --asw-text:#e6e0e9;--asw-text-secondary:#cac4d0;--asw-text-muted:#938f99;
      --asw-border:#49454f;--asw-border-strong:#938f99;
      --asw-bot-bg:#2b2930;--asw-bot-text:#e6e0e9;
      --asw-input-bg:var(--md-surface-3);--asw-input-focus-bg:var(--md-surface-3);
      --asw-success:#7ddaa9;--asw-error:#f2b8b5;--asw-warning:#f5c76a;
      --md-error-container:color-mix(in srgb,#f2b8b5 14%,#141218);
      --asw-header-bg:var(--md-surface-2);
    }
    ...
    /* ─── Ambient scrim glow (kept, softened to M3 tonal wash) ─── */
    .asw-glow{position:fixed;z-index:0;width:320px;height:320px;border-radius:50%;pointer-events:none;background:radial-gradient(circle,color-mix(in srgb,var(--asw-primary) 32%,transparent) 0%,transparent 66%);filter:blur(60px);opacity:0;transform:scale(.6);transition:opacity var(--md-dur-long) var(--md-emph),transform var(--md-dur-long) var(--md-emph-dec)}
    .asw-glow.show{opacity:.42;transform:scale(1)}

    /* ═══ FAB — M3 Large FAB with state layer + ripple ═══ */
    .asw-fab{position:fixed;width:64px;height:64px;border:none;border-radius:20px;background:var(--md-primary-container);color:var(--md-on-primary-container);display:grid;place-items:center;cursor:pointer;overflow:visible;z-index:2;box-shadow:var(--md-e3);transition:box-shadow var(--md-dur-med) var(--md-emph),border-radius var(--md-dur-med) var(--md-emph),background-color var(--md-dur-med) var(--md-emph),color var(--md-dur-med) var(--md-emph),transform var(--md-dur-med) var(--md-emph-dec);animation:asw-fab-in 450ms var(--md-emph-dec) 120ms both}
    /* state layer */
    .asw-fab::before{content:"";position:absolute;inset:0;border-radius:inherit;background:currentColor;opacity:0;transition:opacity var(--md-dur-short) linear;pointer-events:none}
    .asw-fab:hover::before{opacity:.08}
    .asw-fab:focus-visible::before{opacity:.10}
    .asw-fab:active::before{opacity:.12}
    .asw-fab:hover{box-shadow:var(--md-e4)}
    .asw-fab:active{box-shadow:var(--md-e3)}
    .asw-fab.active{background:var(--md-surface-3);color:var(--asw-text-secondary);border-radius:50%;box-shadow:var(--md-e1)}
    ...
    .asw-fab-icon-main,.asw-fab-icon-close{position:absolute;inset:0;display:grid;place-items:center;transition:opacity var(--md-dur-short) linear,transform var(--md-dur-med) var(--md-emph)}
    .asw-fab-icon-close{opacity:0;transform:rotate(-45deg) scale(.6)}
    .asw-fab.active .asw-fab-icon-main{opacity:0;transform:rotate(45deg) scale(.6)}
    .asw-fab.active .asw-fab-icon-close{opacity:1;transform:rotate(0) scale(1)}
    .asw-fab-custom-icon{width:28px;height:28px;border-radius:8px;object-fit:contain}
    .asw-fab-pulse{position:absolute;inset:0;border-radius:inherit;border:2px solid color-mix(in srgb,var(--asw-primary) 55%,transparent);opacity:0;animation:asw-ping 2.8s var(--md-emph-dec) 1.2s infinite;pointer-events:none}
    .asw-fab.active .asw-fab-pulse{display:none}
    .asw-fab-label{position:absolute;inset-inline-end:calc(100% + 12px);top:50%;transform:translateY(-50%) scale(.9);transform-origin:right center;white-space:nowrap;padding:8px 14px;border-radius:var(--md-shape-s);background:#322f35;color:#f5eff7;font-family:inherit;font-size:12px;font-weight:500;letter-spacing:.02em;box-shadow:var(--md-e2);opacity:0;pointer-events:none;transition:opacity var(--md-dur-short) linear,transform var(--md-dur-med) var(--md-emph-dec)}
    .asw.dark .asw-fab-label{background:#e6e0e9;color:#322f35}
    ...
    .asw-fab-badge{position:absolute;top:-2px;inset-inline-end:-2px;min-width:16px;height:16px;padding:0 4px;border-radius:var(--md-shape-full);background:#b3261e;color:#fff;font-size:11px;font-weight:500;line-height:1;display:grid;place-items:center;border:2px solid var(--md-surface);pointer-events:none}
    .asw.dark .asw-fab-badge{background:#f2b8b5;color:#601410;border-color:var(--md-surface)}
    .asw-fab-badge.asw-pop{animation:asw-pop 250ms var(--md-emph-dec)}
    .asw-fab-badge[hidden]{display:none}
    @keyframes asw-fab-in{0%{transform:scale(.3);opacity:0}100%{transform:scale(1);opacity:1}}
    @keyframes asw-ping{0%{transform:scale(1);opacity:.5}70%,100%{transform:scale(1.4);opacity:0}}
    @keyframes asw-pop{0%{transform:scale(0)}100%{transform:scale(1)}}

    /* ═══ Teaser — M3 elevated card ═══ */
    .asw-teaser{position:fixed;z-index:1;max-width:264px;display:flex;align-items:flex-start;gap:12px;padding:16px;border-radius:var(--md-shape-l);background:var(--md-surface-3);color:var(--asw-text);box-shadow:var(--md-e2);cursor:pointer;opacity:0;transform:translateY(8px) scale(.96);transition:opacity var(--md-dur-med) linear,transform var(--md-dur-long) var(--md-emph-dec),box-shadow var(--md-dur-short) linear;pointer-events:none}
    .asw-teaser:hover{box-shadow:var(--md-e3)}
    .asw-teaser.show{opacity:1;transform:translateY(0) scale(1);pointer-events:auto}
    .asw-teaser-orb{width:40px;height:40px;flex:none;border-radius:50%;background:var(--md-secondary-container);color:var(--md-on-secondary-container);display:grid;place-items:center}
    .asw-teaser-orb svg{width:20px;height:20px}
    .asw-teaser-body{min-width:0;padding-inline-end:16px}
    .asw-teaser-title{font-size:14px;font-weight:500;letter-spacing:.01em;color:var(--asw-text)}
    .asw-teaser-text{font-size:12px;color:var(--asw-text-secondary);line-height:1.6;margin-top:4px}
    .asw-teaser-close{position:absolute;top:6px;inset-inline-start:6px;width:28px;height:28px;border:none;border-radius:50%;background:transparent;color:var(--asw-text-muted);display:grid;place-items:center;cursor:pointer;opacity:0;transition:opacity var(--md-dur-short) linear,background-color var(--md-dur-short) linear}
    .asw-teaser:hover .asw-teaser-close,.asw-teaser-close:focus-visible{opacity:1}
    .asw-teaser-close:hover{background:color-mix(in srgb,var(--asw-text) 8%,transparent);color:var(--asw-text)}
    .asw-teaser-close svg{width:12px;height:12px}
    @media(max-width:520px){.asw-teaser{display:none}}

    /* ═══ Panel — M3 elevated surface, container transform ═══ */
    .asw-panel{position:fixed;z-index:3;width:var(--asw-panel-w);height:var(--asw-panel-h);max-width:calc(100vw - 20px);max-height:calc(100dvh - 104px);display:flex;flex-direction:column;overflow:hidden;background:var(--md-surface-1);border-radius:var(--asw-radius);box-shadow:var(--md-e3);opacity:0;transform:translateY(24px) scale(.9);transform-origin:bottom right;pointer-events:none;visibility:hidden;transition:opacity 200ms var(--md-emph-acc),transform 250ms var(--md-emph-acc),visibility 0s linear 250ms}
    .asw-panel.open{opacity:1;transform:translateY(0) scale(1);pointer-events:auto;visibility:visible;transition:opacity 200ms var(--md-emph-dec),transform 400ms var(--md-emph-dec),visibility 0s}
    ...
    .asw-header{position:relative;z-index:4;flex:none;display:flex;align-items:center;gap:12px;padding:12px 16px;min-height:72px;background:var(--md-surface-3);color:var(--asw-text)}
    ...
    .asw-avatar{width:40px;height:40px;border-radius:50%;display:grid;place-items:center;background:var(--md-primary-container);color:var(--md-on-primary-container);font-size:16px;font-weight:500;overflow:hidden}
    .asw-avatar img{width:100%;height:100%;border-radius:inherit;object-fit:cover}
    .asw-status{position:absolute;bottom:0;inset-inline-end:0;width:12px;height:12px;border-radius:50%;background:#33a06f;border:2px solid var(--md-surface-3)}
    .asw-info{min-width:0;flex:1}
    .asw-title-row{display:flex;align-items:center;gap:8px;min-width:0}
    .asw-title{font-size:16px;font-weight:500;letter-spacing:0;line-height:1.5;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--asw-text)}
    .asw-ai-chip{flex:none;display:inline-flex;align-items:center;gap:4px;height:22px;padding:0 8px;border-radius:var(--md-shape-s);font-size:10px;font-weight:500;letter-spacing:.04em;color:var(--md-on-secondary-container);background:var(--md-secondary-container)}
    .asw-ai-chip svg{width:11px;height:11px}
    .asw-badge{display:inline-flex;align-items:center;gap:5px;flex:none;height:22px;padding:0 8px;border-radius:var(--md-shape-s);background:transparent;border:1px solid var(--asw-border);font-size:10px;font-weight:500;letter-spacing:.03em;color:var(--asw-text-secondary);white-space:nowrap}
    .asw-badge-dot{width:6px;height:6px;border-radius:50%;background:#33a06f}
    .asw-subtitle{font-size:12px;font-weight:400;letter-spacing:.02em;color:var(--asw-text-secondary);margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}

    /* M3 icon buttons */
    .asw-actions{position:relative;display:flex;gap:2px;flex:none}
    .asw-header-btn{position:relative;width:40px;height:40px;border:none;border-radius:50%;background:transparent;color:var(--asw-text-secondary);display:grid;place-items:center;cursor:pointer;transition:color var(--md-dur-short) linear}
    .asw-header-btn::before{content:"";position:absolute;inset:0;border-radius:50%;background:currentColor;opacity:0;transition:opacity var(--md-dur-short) linear}
    ...
    .asw-header-btn:hover{color:var(--asw-text)}
    .asw-header-btn svg{width:20px;height:20px;position:relative;transition:transform var(--md-dur-long) var(--md-emph)}
    #asw-clear-history:hover svg{transform:rotate(-180deg)}

    /* ═══ Offline banner ═══ */
    .asw-netstatus{position:relative;z-index:3;flex:none;display:flex;align-items:center;justify-content:center;gap:8px;padding:10px 16px;background:color-mix(in srgb,var(--asw-warning) 16%,var(--md-surface));color:var(--asw-warning);font-size:12px;font-weight:500;letter-spacing:.02em}
    .asw-netstatus[hidden]{display:none}
    .asw-netstatus svg{width:16px;height:16px;flex:none}

    /* ═══ Messages ═══ */
    .asw-messages{position:relative;z-index:1;flex:1;min-height:0;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;padding:16px 16px 8px;display:flex;flex-direction:column;gap:12px;scroll-behavior:smooth;scrollbar-width:thin;scrollbar-color:var(--asw-border-strong) transparent;background:var(--md-surface-1)}
    .asw-messages::-webkit-scrollbar{width:6px}
    .asw-messages::-webkit-scrollbar-track{background:transparent}
    .asw-messages::-webkit-scrollbar-thumb{background:color-mix(in srgb,var(--asw-text) 22%,transparent);border-radius:var(--md-shape-full)}

    .asw-row{position:relative;z-index:1;display:flex;flex-wrap:wrap;align-items:flex-end;gap:8px;max-width:86%;animation:asw-in 300ms var(--md-emph-dec) both}
    .asw-row.user{align-self:flex-end;flex-direction:row-reverse}
    .asw-row.bot{align-self:flex-start}
    .asw-row.bot + .asw-row.bot,.asw-row.user + .asw-row.user{margin-top:-8px}
    ...
    .asw-row.bot .asw-msg-avatar{background:var(--md-primary-container);color:var(--md-on-primary-container)}
    .asw-row.bot .asw-msg-avatar img{width:100%;height:100%;object-fit:cover}
    .asw-row.user .asw-msg-avatar{background:var(--md-secondary-container);color:var(--md-on-secondary-container)}
    .asw-no-avatar .asw-msg-avatar{display:none}

    .asw-divider{position:relative;z-index:1;align-self:center;display:flex;align-items:center;gap:8px;margin:4px 0;padding:4px 12px;border-radius:var(--md-shape-full);background:var(--md-surface-3);color:var(--asw-text-secondary);font-size:11px;font-weight:500;letter-spacing:.04em;animation:asw-in 250ms var(--md-emph-dec) both}
    ...
    .asw-bubble{padding:12px 16px;font-size:var(--asw-font-size,14px);line-height:1.8;letter-spacing:.01em;word-break:break-word;white-space:pre-wrap;overflow-wrap:anywhere;position:relative;text-align:start}
    .asw-row.bot .asw-bubble{background:var(--asw-bot-bg);color:var(--asw-bot-text);border-radius:16px 16px 4px 16px}
    .asw-row.user .asw-bubble{background:var(--asw-user-bg);color:var(--asw-user-text);border-radius:16px 16px 16px 4px}
    .asw[data-bubble="rounded"] .asw-row.bot .asw-bubble{border-radius:16px 16px 4px 16px}
    .asw[data-bubble="rounded"] .asw-row.user .asw-bubble{border-radius:16px 16px 16px 4px}
    .asw[data-bubble="sharp"] .asw-bubble{border-radius:var(--md-shape-xs)}
    .asw[data-bubble="pill"] .asw-bubble{border-radius:var(--md-shape-xl);padding:12px 20px}

    .asw-bubble strong{font-weight:600}
    .asw-bubble em{font-style:italic}
    .asw-bubble a{color:var(--asw-primary);font-weight:500;text-decoration:underline;text-underline-offset:3px}
    .asw.dark .asw-bubble a{color:color-mix(in srgb,var(--asw-primary) 35%,#ffffff)}
    ...
    .asw-table-wrap{margin:12px 0;border:1px solid var(--asw-border);border-radius:var(--md-shape-m);overflow:hidden;overflow-x:auto;background:var(--md-surface);direction:rtl}
    .asw-table-wrap table{border-collapse:collapse;width:100%;font-size:12.5px;line-height:1.7}
    .asw-table-wrap th{background:var(--md-surface-2);font-weight:500;text-align:start;padding:10px 14px;white-space:nowrap;color:var(--asw-text)}
    .asw-table-wrap td{padding:10px 14px;border-top:1px solid var(--asw-border);color:var(--asw-text-secondary)}
    .asw-table-wrap tbody tr:hover{background:color-mix(in srgb,var(--asw-primary) 6%,transparent)}
    .asw-table-wrap th[style*="center"],.asw-table-wrap td[style*="center"]{text-align:center}

    .asw-code-wrap{position:relative;margin:12px 0;border-radius:var(--md-shape-m);overflow:hidden;background:#1d1b20;border:1px solid #49454f;direction:ltr;text-align:left}
    .asw-code-header{display:flex;align-items:center;gap:8px;padding:8px 12px;background:#211f26;border-bottom:1px solid #49454f}
    .asw-code-dots{display:flex;gap:5px;flex:none}
    .asw-code-dots span{width:8px;height:8px;border-radius:50%;background:#49454f}
    .asw-code-lang{font-size:11px;color:#cac4d0;font-weight:500;letter-spacing:.08em;text-transform:uppercase;margin-left:4px}
    .asw-code-copy{margin-left:auto;display:inline-flex;align-items:center;gap:6px;height:28px;padding:0 12px;border:1px solid #79747e;border-radius:var(--md-shape-full);background:transparent;color:#cac4d0;font-size:11px;font-weight:500;cursor:pointer;transition:background-color var(--md-dur-short) linear,color var(--md-dur-short) linear;font-family:inherit}
    .asw-code-copy:hover{background:rgba(230,224,233,.08);color:#e6e0e9}
    .asw-code-copy.copied{color:#7ddaa9;border-color:#7ddaa9}
    .asw-code-copy svg{width:13px;height:13px}
    .asw-code-block{margin:0;padding:14px 16px;overflow-x:auto;font-family:ui-monospace,'Roboto Mono','JetBrains Mono',Consolas,monospace;font-size:12.5px;line-height:1.7;color:#e6e0e9;white-space:pre;tab-size:2}
    .asw-code-block::-webkit-scrollbar{height:6px}
    .asw-code-block::-webkit-scrollbar-thumb{background:#49454f;border-radius:var(--md-shape-full)}
    .asw-code-block code{font-family:inherit}
    .asw-bubble code:not(.asw-code-block code){padding:2px 6px;border-radius:var(--md-shape-xs);background:color-mix(in srgb,var(--asw-text) 8%,transparent);font-family:ui-monospace,'Roboto Mono',monospace;font-size:.88em;color:inherit;white-space:pre-wrap}

    .asw-time{padding:4px 4px 0;font-size:11px;color:var(--asw-text-muted);font-weight:400;letter-spacing:.04em}
    .asw-row.user .asw-time{text-align:left}

    /* ═══ Feedback ═══ */
    .asw-feedback{display:flex;align-items:center;gap:0;margin-top:4px;opacity:0;transition:opacity var(--md-dur-short) linear}
    ...
    .asw-fb-btn{position:relative;width:32px;height:32px;border:none;border-radius:50%;background:transparent;color:var(--asw-text-muted);cursor:pointer;display:grid;place-items:center;transition:color var(--md-dur-short) linear}
    .asw-fb-btn::before{content:"";position:absolute;inset:2px;border-radius:50%;background:currentColor;opacity:0;transition:opacity var(--md-dur-short) linear}
    .asw-fb-btn:hover::before{opacity:.08}
    .asw-fb-btn:active::before{opacity:.12}
    .asw-fb-btn:hover{color:var(--asw-text)}
    ...
    .asw-retry{position:relative;display:inline-flex;align-items:center;gap:8px;margin-top:10px;height:36px;padding:0 16px;border-radius:var(--md-shape-full);border:1px solid var(--asw-error);background:transparent;color:var(--asw-error);font-family:inherit;font-size:13px;font-weight:500;letter-spacing:.02em;cursor:pointer;overflow:hidden}
    .asw-retry::before{content:"";position:absolute;inset:0;background:currentColor;opacity:0;transition:opacity var(--md-dur-short) linear}
    ...
    /* ═══ Typing ═══ */
    .asw-typing{position:relative;z-index:1;flex:none;display:flex;align-items:center;gap:10px;padding:0 16px 12px;margin-top:-4px}
    .asw-typing[hidden]{display:none}
    .asw-typing-ava{width:32px;height:32px;flex:none;border-radius:50%;background:var(--md-primary-container);color:var(--md-on-primary-container);display:grid;place-items:center}
    .asw-typing-ava svg{width:14px;height:14px}
    .asw-typing-bubble{display:flex;align-items:center;gap:6px;padding:14px 16px;background:var(--asw-bot-bg);border-radius:16px 16px 4px 16px}
    .asw-typing-dot{width:7px;height:7px;border-radius:50%;background:var(--asw-text-secondary);animation:asw-bounce 1.2s var(--md-standard) infinite}
    .asw-typing-dot:nth-child(2){animation-delay:.15s}
    .asw-typing-dot:nth-child(3){animation-delay:.3s}
    @keyframes asw-bounce{0%,100%{transform:translateY(0);opacity:.4}50%{transform:translateY(-5px);opacity:1}}
    .asw-typing-label{font-size:12px;font-weight:500;letter-spacing:.02em;color:var(--asw-text-muted)}

    /* ═══ Hero ═══ */
    .asw-hero{position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;text-align:center;gap:16px;padding:32px 16px 12px;transition:opacity var(--md-dur-med) linear,transform var(--md-dur-med) var(--md-emph-acc)}
    .asw-hero>*{animation:asw-in 350ms var(--md-emph-dec) both}
    ...
    .asw-hero-spot{position:absolute;top:0;left:50%;transform:translateX(-50%);width:260px;height:160px;background:radial-gradient(ellipse at center,color-mix(in srgb,var(--asw-primary) 14%,transparent),transparent 70%);pointer-events:none}
    .asw-hero-orb-wrap{position:relative}
    .asw-hero-orb{position:relative;width:80px;height:80px;border-radius:50%;background:var(--md-primary-container);color:var(--md-on-primary-container);display:grid;place-items:center;box-shadow:var(--md-e1)}
    .asw-hero-orb svg{width:34px;height:34px}
    .asw-hero-orb img{width:100%;height:100%;border-radius:inherit;object-fit:cover}
    .asw-hero-p{position:absolute;border-radius:50%;background:var(--md-secondary-container)}
    .asw-hero-p.p1{width:10px;height:10px;top:-8px;inset-inline-start:2px}
    .asw-hero-p.p2{width:7px;height:7px;top:14px;inset-inline-end:-16px}
    .asw-hero-p.p3{width:6px;height:6px;bottom:-4px;inset-inline-start:-14px}
    .asw-hero-text{font-size:16px;font-weight:400;color:var(--asw-text);line-height:1.9;max-width:300px;letter-spacing:.01em}
    .asw-hero-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;width:100%;max-width:344px}
    .asw-hero-grid .asw-suggestion{position:relative;justify-content:flex-start;text-align:start;white-space:normal;height:auto;border-radius:var(--md-shape-m);padding:14px;line-height:1.6;font-size:12.5px;gap:8px;background:var(--md-surface-2);border:1px solid var(--asw-border)}
    ...
    .asw-card-arrow{margin-inline-start:auto;flex:none;color:var(--asw-primary);opacity:.7}
    .asw-card-arrow svg{width:16px;height:16px;display:block}
    @media(max-width:400px){.asw-hero-grid{grid-template-columns:1fr}}

    /* ═══ Suggestions — M3 assist chips ═══ */
    .asw-suggestions{position:relative;z-index:2;flex:none;display:flex;gap:8px;align-items:center;overflow-x:auto;padding:8px 16px;scrollbar-width:none;-webkit-overflow-scrolling:touch}
    .asw-suggestions::-webkit-scrollbar{display:none}
    .asw-suggestions[hidden]{display:none}
    .asw-suggestion{position:relative;display:inline-flex;align-items:center;gap:8px;flex:0 0 auto;height:32px;padding:0 16px;border-radius:var(--md-shape-s);border:1px solid var(--asw-border);background:transparent;color:var(--asw-text);font-family:inherit;font-size:13px;font-weight:500;letter-spacing:.02em;cursor:pointer;white-space:nowrap;text-decoration:none;overflow:hidden;transition:border-color var(--md-dur-short) linear,background-color var(--md-dur-short) linear}
    .asw-suggestion::before{content:"";position:absolute;inset:0;background:var(--asw-primary);opacity:0;transition:opacity var(--md-dur-short) linear;pointer-events:none}
    .asw-suggestion:hover::before{opacity:.08}
    .asw-suggestion:active::before{opacity:.12}
    .asw-suggestion>*{position:relative}
    .asw-bubble .asw-suggestion{margin-top:8px}

    /* ═══ Footer / input ═══ */
    .asw-footer{position:relative;z-index:2;flex:none;padding:8px 12px calc(12px + env(safe-area-inset-bottom,0px));background:var(--md-surface-2)}
    .asw-input-wrap{display:flex;align-items:flex-end;gap:4px;padding:6px;padding-inline:20px 6px;border:none;border-radius:var(--md-shape-xl);background:var(--md-surface-4);box-shadow:none;transition:background-color var(--md-dur-short) linear,box-shadow var(--md-dur-short) linear}
    .asw-input-wrap:focus-within{background:var(--md-surface-5);box-shadow:0 0 0 2px var(--asw-primary)}
    .asw-input-wrap.asw-listening{box-shadow:0 0 0 2px var(--asw-error)}

    .asw-input{min-width:0;flex:1;border:0;outline:0;background:transparent;color:var(--asw-text);font-family:inherit;font-size:var(--asw-font-size,14px);line-height:1.6;letter-spacing:.02em;resize:none;padding:10px 0;max-height:132px;overflow-y:auto;scrollbar-width:none}
    .asw-input::-webkit-scrollbar{display:none}
    .asw-input::placeholder{color:var(--asw-text-muted)}

    .asw-wave{display:flex;align-items:center;gap:3px;height:20px;flex:none;padding-inline-end:4px}
    .asw-wave[hidden]{display:none}
    .asw-wave span{width:3px;height:100%;border-radius:var(--md-shape-full);background:var(--asw-error);transform-origin:center;animation:asw-wave .9s var(--md-standard) infinite}
    ...
    .asw-mic{position:relative;width:40px;height:40px;flex:none;border:none;border-radius:50%;background:transparent;color:var(--asw-text-secondary);display:grid;place-items:center;cursor:pointer;transition:color var(--md-dur-short) linear}
    .asw-mic::before{content:"";position:absolute;inset:0;border-radius:50%;background:currentColor;opacity:0;transition:opacity var(--md-dur-short) linear}
    ...
    .asw-send{position:relative;width:44px;height:44px;flex:none;border:none;border-radius:50%;display:grid;place-items:center;background:var(--asw-primary);color:var(--md-on-primary);cursor:pointer;overflow:hidden;transition:background-color var(--md-dur-med) var(--md-emph),opacity var(--md-dur-short) linear,border-radius var(--md-dur-med) var(--md-emph)}
    .asw-send::before{content:"";position:absolute;inset:0;background:#fff;opacity:0;transition:opacity var(--md-dur-short) linear}
    .asw-send:hover::before{opacity:.08}
    .asw-send:active::before{opacity:.12}
    .asw-send:disabled{cursor:default;background:color-mix(in srgb,var(--asw-text) 12%,transparent);color:color-mix(in srgb,var(--asw-text) 38%,transparent)}
    .asw-send:disabled::before{opacity:0}
    .asw-send svg{width:20px;height:20px;position:relative}
    .asw-send.asw-stop{background:var(--asw-error);color:#fff;border-radius:var(--md-shape-m)}
    .asw.dark .asw-send.asw-stop{color:#601410}

    .asw-scrollbtn{position:absolute;top:-52px;inset-inline-end:16px;z-index:5;width:40px;height:40px;border:none;border-radius:var(--md-shape-m);background:var(--md-surface-5);color:var(--asw-text-secondary);display:grid;place-items:center;cursor:pointer;box-shadow:var(--md-e2);opacity:0;transform:scale(.8);pointer-events:none;transition:opacity var(--md-dur-short) linear,transform var(--md-dur-med) var(--md-emph-dec)}
    .asw-scrollbtn.show{opacity:1;transform:scale(1);pointer-events:auto}
    .asw-scrollbtn:hover{box-shadow:var(--md-e3);color:var(--asw-text)}
    ...
    .asw-disclaim{font-size:11px;color:var(--asw-text-muted);text-align:center;letter-spacing:.03em}
    .asw-powered{text-align:center;padding:0;font-size:11px;color:var(--asw-text-muted);font-weight:400;letter-spacing:.03em}
    .asw-powered[hidden]{display:none}
    .asw-powered a{color:var(--asw-primary);text-decoration:none;font-weight:500}
    .asw.dark .asw-powered a{color:color-mix(in srgb,var(--asw-primary) 35%,#ffffff)}
    .asw-powered a:hover{text-decoration:underline}
    .asw-resource-links{display:inline-flex;gap:8px;margin-inline-start:8px}

    /* ═══ Snackbar ═══ */
    .asw-toast{position:absolute;bottom:16px;left:16px;right:16px;top:auto;z-index:50;display:flex;align-items:center;justify-content:center;gap:8px;min-height:48px;padding:12px 16px;border-radius:var(--md-shape-xs);background:#322f35;color:#f5eff7;font-size:13px;font-weight:400;line-height:1.5;letter-spacing:.02em;text-align:center;box-shadow:var(--md-e3);opacity:0;transform:translateY(12px);transition:opacity var(--md-dur-short) linear,transform var(--md-dur-med) var(--md-emph-dec);pointer-events:none}
    .asw.dark .asw-toast{background:#e6e0e9;color:#322f35}
    .asw-toast.show{opacity:1;transform:translateY(0)}
    .asw-toast svg{width:18px;height:18px;flex:none;color:currentColor}

    .asw-bubble.streaming::after{content:"";display:inline-block;width:2px;height:15px;margin-inline-start:2px;background:var(--asw-primary);vertical-align:-2px;animation:asw-blink 1s steps(2) infinite}
    @keyframes asw-blink{50%{opacity:0}}

    /* ═══ Citations ═══ */
    .asw-citations{display:flex;flex-wrap:wrap;align-items:center;gap:8px;margin-top:12px;padding-top:12px;border-top:1px solid var(--asw-border)}
    .asw-citations-label{font-size:11px;font-weight:500;color:var(--asw-text-muted);letter-spacing:.04em}
    .asw-citation{display:inline-flex;align-items:center;gap:6px;height:28px;font-size:12px;font-weight:500;color:var(--asw-text-secondary);background:transparent;border:1px solid var(--asw-border);padding:0 10px;border-radius:var(--md-shape-s);text-decoration:none;max-width:170px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;transition:background-color var(--md-dur-short) linear}
    .asw-citation:hover{background:color-mix(in srgb,var(--asw-primary) 8%,transparent)}
    .asw-citation svg{width:13px;height:13px;flex:none}

    /* ═══ Handoff ═══ */
    .asw-handoff{flex:1 1 100%;display:flex;flex-wrap:wrap;align-items:center;gap:8px;margin-top:4px;padding:16px;border-radius:var(--md-shape-m);border:none;background:var(--md-secondary-container);color:var(--md-on-secondary-container);animation:asw-in 300ms var(--md-emph-dec) both}
    .asw-handoff-label{width:100%;font-size:13px;line-height:1.7;color:inherit;font-weight:400}
    .asw-handoff-btn{position:relative;display:inline-flex;align-items:center;gap:8px;height:36px;padding:0 16px;border-radius:var(--md-shape-full);border:1px solid color-mix(in srgb,currentColor 40%,transparent);background:transparent;color:inherit;font-family:inherit;font-size:13px;font-weight:500;letter-spacing:.02em;cursor:pointer;text-decoration:none;overflow:hidden}
    .asw-handoff-btn::before{content:"";position:absolute;inset:0;background:currentColor;opacity:0;transition:opacity var(--md-dur-short) linear}
    ...
    .asw-rule-hint{margin-top:8px;padding:12px 14px;background:var(--md-surface-3);border-radius:var(--md-shape-m);font-size:12.5px;line-height:1.7;color:var(--asw-text-secondary)}
    .asw-messages > .asw-rule-hint{align-self:center;margin:2px 0 4px;padding:6px 14px;border-radius:var(--md-shape-full);background:var(--md-surface-3);color:var(--asw-text-muted);font-size:11.5px;animation:asw-in 250ms both}

    /* ═══ Bottom sheet ═══ */
    .asw-sheet{position:absolute;inset:0;z-index:40;display:flex;align-items:flex-end;opacity:0;pointer-events:none;transition:opacity var(--md-dur-med) linear}
    .asw-sheet::before{content:"";position:absolute;inset:0;background:rgba(0,0,0,.32)}
    .asw-sheet.open{opacity:1;pointer-events:auto}
    .asw-sheet[hidden]{display:none}
    .asw-sheet-card{position:relative;width:100%;max-height:92%;overflow-y:auto;overscroll-behavior:contain;background:var(--md-surface-3);border-radius:var(--md-shape-xl) var(--md-shape-xl) 0 0;box-shadow:var(--md-e3);padding:12px 24px calc(24px + env(safe-area-inset-bottom,0px));transform:translateY(100%);transition:transform 400ms var(--md-emph-dec);scrollbar-width:thin}
    .asw-sheet.open .asw-sheet-card{transform:translateY(0)}
    .asw-sheet-grip{width:32px;height:4px;border-radius:var(--md-shape-full);background:color-mix(in srgb,var(--asw-text) 40%,transparent);margin:0 auto 16px}
    .asw-sheet-close{position:absolute;top:12px;inset-inline-end:12px;width:40px;height:40px;border:none;border-radius:50%;background:transparent;color:var(--asw-text-secondary);display:grid;place-items:center;cursor:pointer;transition:background-color var(--md-dur-short) linear}
    .asw-sheet-close:hover{background:color-mix(in srgb,var(--asw-text) 8%,transparent);color:var(--asw-text)}
    .asw-sheet-close svg{width:18px;height:18px}
    .asw-sheet-title{font-size:20px;font-weight:400;color:var(--asw-text);letter-spacing:0;line-height:1.4}
    .asw-sheet-desc{font-size:13px;color:var(--asw-text-secondary);line-height:1.8;margin:6px 0 18px;letter-spacing:.02em}

    .asw-chans{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:18px}
    @media(max-width:360px){.asw-chans{grid-template-columns:1fr}}
    .asw-chan{position:relative;display:flex;align-items:center;gap:12px;padding:12px;border-radius:var(--md-shape-m);border:1px solid var(--asw-border);background:transparent;text-decoration:none;overflow:hidden;transition:background-color var(--md-dur-short) linear,border-color var(--md-dur-short) linear}
    .asw-chan::before{content:"";position:absolute;inset:0;background:var(--chan,var(--asw-primary));opacity:0;transition:opacity var(--md-dur-short) linear}
    .asw-chan:hover::before{opacity:.08}
    .asw-chan:active::before{opacity:.12}
    .asw-chan>*{position:relative}
    .asw-chan-ic{width:40px;height:40px;flex:none;border-radius:50%;display:grid;place-items:center;color:#fff;background:var(--chan,var(--asw-primary))}
    .asw-chan-ic svg{width:18px;height:18px}
    .asw-chan-tx{min-width:0;display:flex;flex-direction:column}
    .asw-chan-tx b{font-size:13.5px;font-weight:500;color:var(--asw-text);letter-spacing:.01em}
    .asw-chan-tx i{font-style:normal;font-size:11.5px;color:var(--asw-text-muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;direction:ltr;text-align:end}
    ...
    .asw-flabel{display:flex;align-items:center;gap:4px;font-size:12px;font-weight:500;color:var(--asw-text-secondary);margin-bottom:6px;letter-spacing:.03em}
    .asw-flabel b{color:var(--asw-error);font-weight:500}
    .asw-form input,.asw-form textarea{width:100%;box-sizing:border-box;font-family:inherit;font-size:14px;color:var(--asw-text);padding:14px 16px;border-radius:var(--md-shape-xs);border:1px solid var(--asw-border-strong);background:transparent;outline:none;letter-spacing:.02em;transition:border-color var(--md-dur-short) linear,box-shadow var(--md-dur-short) linear}
    ...
    .asw-form input:focus,.asw-form textarea:focus{border-color:var(--asw-primary);box-shadow:inset 0 0 0 1px var(--asw-primary)}
    .asw-field.invalid input,.asw-field.invalid textarea{border-color:var(--asw-error);box-shadow:inset 0 0 0 1px var(--asw-error)}
    .asw-field.ok input{border-color:var(--asw-success)}
    .asw-ferr{display:none;font-size:12px;font-weight:400;color:var(--asw-error);margin-top:5px;padding-inline-start:16px;letter-spacing:.03em}
    .asw-field.invalid .asw-ferr{display:block}

    .asw-form-submit{position:relative;width:100%;display:inline-flex;align-items:center;justify-content:center;gap:8px;margin-top:6px;height:48px;padding:0 24px;border:none;border-radius:var(--md-shape-full);background:var(--asw-primary);color:var(--md-on-primary);font-family:inherit;font-size:14px;font-weight:500;letter-spacing:.03em;cursor:pointer;overflow:hidden;transition:box-shadow var(--md-dur-short) linear,background-color var(--md-dur-short) linear}
    .asw-form-submit::before{content:"";position:absolute;inset:0;background:#fff;opacity:0;transition:opacity var(--md-dur-short) linear}
    .asw-form-submit:hover::before{opacity:.08}
    .asw-form-submit:active::before{opacity:.12}
    .asw-form-submit:hover{box-shadow:var(--md-e1)}
    .asw-form-submit:disabled{background:color-mix(in srgb,var(--asw-text) 12%,transparent);color:color-mix(in srgb,var(--asw-text) 38%,transparent);cursor:default;box-shadow:none}
    ...
    .asw-spinner{width:16px;height:16px;flex:none;border:2px solid color-mix(in srgb,currentColor 35%,transparent);border-top-color:currentColor;border-radius:50%;animation:asw-rot .8s linear infinite;position:relative}
    @keyframes asw-rot{to{transform:rotate(360deg)}}
    .asw-form-note{display:flex;align-items:center;justify-content:center;gap:6px;font-size:11.5px;color:var(--asw-text-muted);text-align:center;margin-top:12px;line-height:1.7;letter-spacing:.03em}
    .asw-form-note svg{width:13px;height:13px;flex:none;opacity:.8}
    .asw-form-err{display:none;font-size:12.5px;font-weight:400;color:var(--asw-error);margin-top:10px;text-align:center}
    .asw-form-err.show{display:block}

    .asw-form-success{display:flex;flex-direction:column;align-items:center;text-align:center;gap:8px;padding:24px 8px 8px;animation:asw-in 300ms var(--md-emph-dec) both}
    .asw-checkmark{width:64px;height:64px;stroke:var(--asw-success);stroke-width:3;fill:none;stroke-linecap:round;stroke-linejoin:round}
    .asw-checkmark circle{stroke-dasharray:170;stroke-dashoffset:170;animation:asw-draw 600ms var(--md-emph-dec) 80ms forwards}
    .asw-checkmark path{stroke-dasharray:44;stroke-dashoffset:44;animation:asw-draw 400ms var(--md-emph-dec) 560ms forwards}
    @keyframes asw-draw{to{stroke-dashoffset:0}}
    .asw-fs-title{font-size:18px;font-weight:500;color:var(--asw-text);margin-top:8px}
    .asw-fs-desc{font-size:13px;color:var(--asw-text-secondary);line-height:1.9;max-width:270px}
    .asw-fs-close{position:relative;margin-top:14px;height:40px;padding:0 24px;border-radius:var(--md-shape-full);border:none;background:transparent;color:var(--asw-primary);font-family:inherit;font-size:14px;font-weight:500;letter-spacing:.03em;cursor:pointer;overflow:hidden}
    .asw-fs-close::before{content:"";position:absolute;inset:0;background:currentColor;opacity:0;transition:opacity var(--md-dur-short) linear}
    .asw-fs-close:hover::before{opacity:.08}
    .asw-fs-close:active::before{opacity:.12}
    .asw.dark .asw-fs-close{color:color-mix(in srgb,var(--asw-primary) 35%,#ffffff)}
    .asw-leadform-wrap{animation:asw-in 300ms var(--md-emph-dec) both}
    .asw-leadform{position:relative;flex:1 1 100%;margin:4px 0;padding:16px;border-radius:var(--md-shape-m);border:1px solid var(--asw-border);background:var(--md-surface-2);animation:asw-in 300ms var(--md-emph-dec) both}

    .asw-bubble.asw-error-bubble{background:var(--md-error-container);color:var(--asw-error)}

    /* ═══ Responsive ═══ */
    @media(max-width:480px){
      .asw-fab{width:56px;height:56px;border-radius:16px}
      .asw-panel{width:min(var(--asw-panel-w),calc(100vw - 24px));height:min(var(--asw-panel-h),calc(100dvh - 88px))}
      .asw-root.mobile-fullscreen .asw-panel{position:fixed!important;inset:0!important;width:100vw!important;max-width:none!important;height:100vh!important;height:100dvh!important;max-height:none!important;border-radius:0!important;transform:translateY(24px)!important}
    ...
    .asw :focus-visible{outline:3px solid var(--asw-primary);outline-offset:2px}
    .asw ::selection{background:color-mix(in srgb,var(--asw-primary) 24%,transparent)}
