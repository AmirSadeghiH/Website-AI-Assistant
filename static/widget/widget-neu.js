...
(function () {
  ...
  var CSS = `
    /* ═══════════════════════════════════════════════════════════════════════
       AI Support Widget — NEUMORPHISM / SOFT UI
       ───────────────────────────────────────────────────────────────────────
       One surface colour. Everything is that same colour. Form comes only
       from a consistent, single light source in the upper-left:

         EXTRUDED  · -6px -6px 12px LIGHT  +  6px 6px 12px DARK
                     the element pushes out of the background
         INSET     · inset 6px 6px 12px DARK + inset -6px -6px 12px LIGHT
                     the element is pressed into the background
         FLAT      · no shadow — the element is level with the surface

       Rules the whole sheet obeys:
         · The light always comes from the top-left. No exceptions.
         · Backgrounds never differ from the base tone; only shadows differ.
         · Borders are avoided; where present they are 1px of near-surface
           tone used to sharpen an edge, never to draw a box.
         · Pressing a raised control makes it inset. That is the only
           interaction metaphor used anywhere in the widget.
         · Accent colour appears only where meaning requires it.
    ...
    :host{all:initial;--asw-font:'Vazirmatn','Vazir','IRANSans','Nunito',ui-sans-serif,system-ui,sans-serif}
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}

    /* ═══════════════════════════════════════════════════════════════════
       THE SURFACE — one colour, two lights
       ═══════════════════════════════════════════════════════════════════ */
    .asw{
      --asw-primary:#6d7cff;--asw-secondary:#6d7cff;--asw-accent:#6d7cff;
      --accent:var(--asw-primary);

      /* the single base tone everything is made of */
      --surface:#e6e9f0;
      /* the two lights derived from it */
      --light:#ffffff;
      --dark:#bcc2ce;
      ...
      /* status */
      --ok:#4a9a6f;--err:#d1596b;--warn:#c39355;--online:#5fc48f;

      /* ── the neumorphic shadow vocabulary ── */
      --x-sm:-3px -3px 6px var(--light), 3px 3px 6px var(--dark);
      --x-md:-6px -6px 12px var(--light), 6px 6px 12px var(--dark);
      --x-lg:-10px -10px 22px var(--light), 10px 10px 22px var(--dark);
      --x-xl:-16px -16px 34px var(--light), 18px 18px 40px var(--dark);

      --in-sm:inset 2px 2px 4px var(--dark), inset -2px -2px 4px var(--light);
      --in-md:inset 4px 4px 9px var(--dark), inset -4px -4px 9px var(--light);
      --in-lg:inset 7px 7px 15px var(--dark), inset -7px -7px 15px var(--light);

      /* accent-tinted extrusion, for the primary action only */
      --x-accent:-4px -4px 10px color-mix(in srgb,var(--accent) 22%,var(--light)),
                  5px 5px 12px color-mix(in srgb,var(--accent) 42%,var(--dark));

      /* soft geometry — neumorphism needs generous radii */
      ...
      --w-reg:500;--w-med:600;--w-bold:700;

      /* motion — soft, never snappy */
      --ms:160ms;--mm:260ms;--ml:420ms;
      --ease:cubic-bezier(.4,0,.2,1);
      --ease-out:cubic-bezier(.16,1,.3,1);

      /* aliases consumed by the unchanged JS */
      --asw-bg:var(--surface);--asw-surface:var(--surface);--asw-surface-2:var(--surface);
      --asw-text:var(--ink);--asw-text-secondary:var(--ink-2);--asw-text-muted:var(--ink-3);
      --asw-border:transparent;--asw-border-strong:var(--dark);
      --asw-bot-bg:var(--surface);--asw-bot-text:var(--ink);
      --asw-user-bg:var(--accent);--asw-user-bg-2:var(--accent);--asw-user-text:var(--on-accent);
      --asw-input-bg:var(--surface);--asw-input-focus-bg:var(--surface);
      --asw-shadow:var(--x-xl);--asw-header-bg:var(--surface);
    ...
    /* dark — same principle, the surface just gets darker.
       The light stays top-left; it simply becomes a lifted grey. */
    .asw.dark{
      --surface:#2b2f3a;
      --light:#353a47;
      --dark:#20232b;
      ...
      --ink-2:#9aa1b5;
      --ink-3:#6f7688;
      --ok:#5fc48f;--err:#e8798a;--warn:#dbb072;--online:#5fc48f;
    }
    ...
    .asw-glow{position:fixed;z-index:0;width:300px;height:300px;border-radius:var(--r-pill);pointer-events:none;
    ...
    .asw-glow.show{opacity:.45;transform:scale(1)}

    /* ═══════════════════════════════════════════════════════════════════
       LAUNCHER — extruded disc; press makes it inset
       ═══════════════════════════════════════════════════════════════════ */
    .asw-fab{position:fixed;width:64px;height:64px;border:none;border-radius:var(--r-pill);cursor:pointer;z-index:2;
      display:grid;place-items:center;
      background:var(--surface);
      color:var(--accent);
      box-shadow:var(--x-md);
      transition:box-shadow var(--mm) var(--ease),color var(--mm) var(--ease),transform var(--mm) var(--ease-out);
      animation:nu-in var(--ml) var(--ease-out) 100ms both}
    .asw-fab:hover{box-shadow:var(--x-lg)}
    .asw-fab:active{box-shadow:var(--in-md);transform:scale(.98)}
    .asw-fab.active{box-shadow:var(--in-md);color:var(--ink-2)}
    .asw-fab svg{width:25px;height:25px;position:relative}
    ...
    .asw-fab-custom-icon{width:28px;height:28px;border-radius:var(--r-sm);object-fit:contain}
    .asw-fab-pulse{position:absolute;inset:0;border-radius:inherit;
      box-shadow:0 0 0 0 color-mix(in srgb,var(--accent) 40%,transparent);
      opacity:0;animation:nu-ping 2.8s var(--ease) 1.4s infinite;pointer-events:none}
    .asw-fab.active .asw-fab-pulse{display:none}

    /* badge — a small extruded pill carrying the accent */
    .asw-fab-badge{position:absolute;top:-2px;inset-inline-end:-2px;min-width:22px;height:22px;padding:0 var(--sp-2);
      ...
      box-shadow:var(--x-sm);pointer-events:none}
    .asw-fab-badge[hidden]{display:none}
    .asw-fab-badge.asw-pop{animation:nu-pop var(--mm) var(--ease-out)}

    /* tooltip — a soft extruded chip */
    .asw-fab-label{position:absolute;inset-inline-end:calc(100% + var(--sp-4));top:50%;
      ...
      background:var(--surface);color:var(--ink);box-shadow:var(--x-sm);
      font-family:inherit;font-size:var(--t-sm);font-weight:var(--w-med);
      opacity:0;pointer-events:none;
      transition:opacity var(--ms) var(--ease),transform var(--mm) var(--ease-out)}
    .corner-bl .asw-fab-label,.corner-tl .asw-fab-label{inset-inline-end:auto;inset-inline-start:calc(100% + var(--sp-4));transform:translateY(-50%) translateX(-6px)}
    ...
    /* ═══════════════════════════════════════════════════════════════════
       TEASER — an extruded card
       ═══════════════════════════════════════════════════════════════════ */
    .asw-teaser{position:fixed;z-index:1;max-width:268px;display:flex;align-items:flex-start;gap:var(--sp-3);
      padding:var(--sp-4) var(--sp-5);border-radius:var(--r-lg);
      background:var(--surface);box-shadow:var(--x-md);border:none;
      cursor:pointer;opacity:0;transform:translateY(10px);
      transition:opacity var(--mm) var(--ease),transform var(--mm) var(--ease-out),box-shadow var(--mm) var(--ease);
      pointer-events:none}
    .asw-teaser:hover{box-shadow:var(--x-lg)}
    .asw-teaser.show{opacity:1;transform:translateY(0);pointer-events:auto}
    .asw-teaser-orb{width:36px;height:36px;flex:none;border-radius:var(--r-pill);
      background:var(--surface);color:var(--accent);display:grid;place-items:center;box-shadow:var(--x-sm)}
    ...
    .asw-teaser-close{position:absolute;top:var(--sp-2);inset-inline-start:var(--sp-2);width:24px;height:24px;
      border:none;border-radius:var(--r-pill);background:var(--surface);color:var(--ink-3);
      display:grid;place-items:center;cursor:pointer;opacity:0;box-shadow:var(--x-sm);
      transition:opacity var(--ms) var(--ease),box-shadow var(--ms) var(--ease)}
    .asw-teaser:hover .asw-teaser-close,.asw-teaser-close:focus-visible{opacity:1}
    .asw-teaser-close:active{box-shadow:var(--in-sm)}
    .asw-teaser-close svg{width:10px;height:10px}
    @media(max-width:520px){.asw-teaser{display:none}}

    /* ═══════════════════════════════════════════════════════════════════
       PANEL — the largest extrusion; everything else sits on it
       ═══════════════════════════════════════════════════════════════════ */
    ...
    /* ═══════════════════════════════════════════════════════════════════
       HEADER — level with the surface, no divider line
       ═══════════════════════════════════════════════════════════════════ */
    .asw-header{position:relative;z-index:4;flex:none;display:flex;align-items:center;gap:var(--sp-3);
      padding:var(--sp-5) var(--sp-5) var(--sp-4);background:var(--surface);border:none}
    ...
    /* avatar sits in a pressed well */
    .asw-avatar{width:42px;height:42px;border-radius:var(--r-pill);display:grid;place-items:center;overflow:hidden;
    ...
    .asw-status{position:absolute;bottom:0;inset-inline-end:0;width:12px;height:12px;border-radius:var(--r-pill);
      ...
      box-shadow:0 0 0 1px color-mix(in srgb,var(--dark) 60%,transparent)}
    ...
    /* chips — pressed into the surface */
    ...
    .asw-badge-dot{width:6px;height:6px;border-radius:var(--r-pill);background:var(--online);flex:none;
      box-shadow:0 0 4px color-mix(in srgb,var(--online) 70%,transparent)}

    /* header keys — extruded, inset on press */
    .asw-actions{position:relative;display:flex;gap:var(--sp-2);flex:none}
    .asw-header-btn{width:34px;height:34px;border:none;border-radius:var(--r-pill);
      background:var(--surface);color:var(--ink-2);display:grid;place-items:center;cursor:pointer;
      box-shadow:var(--x-sm);
      transition:box-shadow var(--ms) var(--ease),color var(--ms) var(--ease)}
    .asw-header-btn:hover{color:var(--accent)}
    .asw-header-btn:active{box-shadow:var(--in-sm);color:var(--ink-2)}
    ...
    /* ═══════════════════════════════════════════════════════════════════
       MESSAGES — a deep well carved into the panel
       ═══════════════════════════════════════════════════════════════════ */
    .asw-messages{position:relative;z-index:1;flex:1;min-height:0;overflow-y:auto;overflow-x:hidden;
      ...
      box-shadow:var(--in-md);
      scroll-behavior:smooth;scrollbar-width:thin;scrollbar-color:var(--dark) transparent}
    .asw-messages::-webkit-scrollbar{width:8px}
    .asw-messages::-webkit-scrollbar-track{background:transparent}
    .asw-messages::-webkit-scrollbar-thumb{border-radius:var(--r-pill);background:var(--dark);
      border:2px solid transparent;background-clip:padding-box}

    .asw-row{position:relative;z-index:1;display:flex;flex-wrap:wrap;align-items:flex-end;gap:var(--sp-2);
      max-width:88%;animation:nu-rise var(--ml) var(--ease-out) both}
    ...
    .asw-msg-avatar{width:30px;height:30px;flex:0 0 30px;border-radius:var(--r-pill);display:grid;place-items:center;
      font-size:10px;font-weight:var(--w-bold);overflow:hidden;
      background:var(--surface);box-shadow:var(--x-sm)}
    .asw-row.bot .asw-msg-avatar{color:var(--accent)}
    .asw-row.bot .asw-msg-avatar img{width:100%;height:100%;object-fit:cover}
    .asw-row.user .asw-msg-avatar{color:var(--ink-2)}
    .asw-no-avatar .asw-msg-avatar{display:none}

    /* divider — an engraved groove */
    .asw-divider{position:relative;z-index:1;align-self:center;display:flex;align-items:center;gap:var(--sp-3);
      margin:var(--sp-1) 0;padding:var(--sp-1) var(--sp-4);border-radius:var(--r-pill);
      background:var(--surface);box-shadow:var(--in-sm);
      color:var(--ink-3);font-size:10.5px;font-weight:var(--w-bold);letter-spacing:.03em;
      animation:nu-fade var(--mm) var(--ease-out) both}

    /* ── BUBBLES ──
       bot  : extruded from the well
       user : the one accent-filled element, also extruded */
    ...
    .asw-row.bot .asw-bubble{background:var(--surface);color:var(--ink);border:none;
      border-radius:var(--r-md) var(--r-md) var(--sp-1) var(--r-md);
      box-shadow:var(--x-sm)}
    .asw-row.user .asw-bubble{background:var(--accent);color:var(--on-accent);border:none;
      border-radius:var(--r-md) var(--r-md) var(--r-md) var(--sp-1);
      box-shadow:var(--x-accent)}
    ...
    .asw-bubble.asw-error-bubble{background:var(--surface);color:var(--err);box-shadow:var(--in-sm)}
    ...
    .asw-bubble a:hover{border-bottom-color:var(--accent)}
    ...
    .asw-bubble code:not(.asw-code-block code){padding:2px var(--sp-2);border-radius:var(--sp-2);
      background:var(--surface);box-shadow:var(--in-sm);border:none;
      font-family:ui-monospace,'JetBrains Mono',Consolas,monospace;font-size:.86em;color:var(--accent);white-space:pre-wrap}
    .asw-row.user .asw-bubble code{background:rgba(255,255,255,.2);box-shadow:none;color:#fff}

    /* table — inset panel */
    .asw-table-wrap{margin:var(--sp-3) 0;border:none;border-radius:var(--r-md);overflow:hidden;overflow-x:auto;
      background:var(--surface);box-shadow:var(--in-sm);direction:rtl}
    ...
    /* code — a recessed slab */
    .asw-code-wrap{position:relative;margin:var(--sp-3) 0;border-radius:var(--r-md);overflow:hidden;
      background:var(--surface);border:none;box-shadow:var(--in-md);direction:ltr;text-align:left}
    ...
    .asw-code-dots span{width:8px;height:8px;border-radius:var(--r-pill);background:var(--surface);
      box-shadow:var(--in-sm)}
    .asw-code-lang{font-size:10px;color:var(--ink-3);font-weight:var(--w-bold);letter-spacing:.1em;
    ...
    .asw-code-copy{margin-left:auto;display:inline-flex;align-items:center;gap:var(--sp-1);height:24px;
      padding:0 var(--sp-2);border:none;border-radius:var(--r-pill);
      background:var(--surface);color:var(--ink-2);box-shadow:var(--x-sm);
      font-size:10px;font-weight:var(--w-med);cursor:pointer;font-family:inherit;
      transition:box-shadow var(--ms) var(--ease),color var(--ms) var(--ease)}
    .asw-code-copy:active{box-shadow:var(--in-sm)}
    ...
    .asw-code-block{margin:0;padding:var(--sp-3) var(--sp-4);overflow-x:auto;
      ...
      color:var(--ink);white-space:pre;tab-size:2;background:none}
    .asw-code-block::-webkit-scrollbar{height:6px}
    .asw-code-block::-webkit-scrollbar-thumb{background:var(--dark);border-radius:var(--r-pill)}
    ...
    .asw-fb-btn{width:28px;height:28px;border:none;border-radius:var(--r-pill);
      background:var(--surface);color:var(--ink-3);cursor:pointer;display:grid;place-items:center;
      box-shadow:var(--x-sm);
      transition:box-shadow var(--ms) var(--ease),color var(--ms) var(--ease)}
    .asw-fb-btn:hover{color:var(--accent)}
    .asw-fb-btn:active{box-shadow:var(--in-sm)}
    .asw-fb-btn.active{box-shadow:var(--in-sm);color:var(--accent)}
    .asw-fb-btn.active.thumbs-up,.asw-fb-btn.copied{color:var(--ok);box-shadow:var(--in-sm)}
    .asw-fb-btn.active.thumbs-down{color:var(--err);box-shadow:var(--in-sm)}
    ...
    .asw-retry{display:inline-flex;align-items:center;justify-content:center;gap:var(--sp-2);margin-top:var(--sp-3);
      height:32px;padding:0 var(--sp-4);border:none;border-radius:var(--r-pill);
      background:var(--surface);color:var(--err);box-shadow:var(--x-sm);
      font-family:inherit;font-size:var(--t-xs);font-weight:var(--w-bold);cursor:pointer;
      transition:box-shadow var(--ms) var(--ease)}
    .asw-retry:active{box-shadow:var(--in-sm)}
    ...
    .asw-typing-ava{width:28px;height:28px;flex:none;border-radius:var(--r-pill);
      background:var(--surface);color:var(--accent);display:grid;place-items:center;box-shadow:var(--x-sm)}
    .asw-typing-ava svg{width:12px;height:12px}
    .asw-typing-bubble{display:flex;align-items:center;gap:var(--sp-1);padding:var(--sp-3) var(--sp-4);
      background:var(--surface);border:none;border-radius:var(--r-md);box-shadow:var(--x-sm)}
    .asw-typing-dot{width:7px;height:7px;border-radius:var(--r-pill);background:var(--surface);
      box-shadow:var(--in-sm);animation:nu-bounce 1.2s var(--ease) infinite}
    ...
    .asw-bubble.streaming::after{content:"";display:inline-block;width:3px;height:15px;margin-inline-start:3px;
      border-radius:var(--r-pill);background:var(--accent);vertical-align:-2px;animation:nu-blink 1s steps(2) infinite}
    ...
    .asw-hero>*{animation:nu-rise var(--ml) var(--ease-out) both}
    ...
    /* a raised dome in a ring */
    .asw-hero-orb{position:relative;width:78px;height:78px;border-radius:var(--r-pill);
      background:var(--surface);color:var(--accent);display:grid;place-items:center;
      box-shadow:var(--x-md)}
    .asw-hero-orb::after{content:"";position:absolute;inset:-9px;border-radius:var(--r-pill);
      box-shadow:var(--in-sm);pointer-events:none}
    .asw-hero-orb svg{width:32px;height:32px;position:relative}
    .asw-hero-orb img{width:100%;height:100%;border-radius:inherit;object-fit:cover}
    .asw-hero-p{position:absolute;border-radius:var(--r-pill);background:var(--surface);box-shadow:var(--x-sm)}
    .asw-hero-p.p1{width:10px;height:10px;top:-14px;inset-inline-start:0}
    .asw-hero-p.p2{width:8px;height:8px;top:16px;inset-inline-end:-22px}
    .asw-hero-p.p3{width:7px;height:7px;bottom:-8px;inset-inline-start:-20px}
    .asw-hero-text{font-size:var(--t-lg);font-weight:var(--w-med);color:var(--ink);line-height:1.9;max-width:290px}
    ...
    /* ═══════════════════════════════════════════════════════════════════
       SUGGESTIONS — a row of soft keys
       ═══════════════════════════════════════════════════════════════════ */
    .asw-suggestions{position:relative;z-index:2;flex:none;display:flex;gap:var(--sp-3);align-items:center;
      overflow-x:auto;padding:var(--sp-4) var(--sp-5) var(--sp-2);background:var(--surface);
    ...
    .asw-suggestion{display:inline-flex;align-items:center;gap:var(--sp-2);flex:0 0 auto;height:34px;
      ...
      transition:box-shadow var(--ms) var(--ease),color var(--ms) var(--ease)}
    .asw-suggestion:hover{color:var(--accent)}
    .asw-suggestion:active{box-shadow:var(--in-sm);color:var(--ink-2)}
    .asw-bubble .asw-suggestion{margin-top:var(--sp-2)}

    /* ═══════════════════════════════════════════════════════════════════
       COMPOSER — a pressed trough holding a raised send key
       ═══════════════════════════════════════════════════════════════════ */
    ...
    .asw-input-wrap{display:flex;align-items:flex-end;gap:var(--sp-2);padding:var(--sp-1);
      padding-inline:var(--sp-5) var(--sp-1);
      border:none;border-radius:var(--r-xl);
      background:var(--surface);
      box-shadow:var(--in-md);
      transition:box-shadow var(--mm) var(--ease)}
    .asw-input-wrap:focus-within{box-shadow:var(--in-lg)}
    .asw-input-wrap.asw-listening{box-shadow:var(--in-md),0 0 0 2px color-mix(in srgb,var(--err) 40%,transparent)}
    ...
    .asw-wave span{width:3px;height:100%;border-radius:var(--r-pill);background:var(--err);
      transform-origin:center;animation:nu-wave .9s var(--ease) infinite}
    ...
    .asw-mic{width:36px;height:36px;flex:none;border:none;border-radius:var(--r-pill);
      background:var(--surface);color:var(--ink-3);display:grid;place-items:center;cursor:pointer;
      box-shadow:var(--x-sm);transition:box-shadow var(--ms) var(--ease),color var(--ms) var(--ease)}
    .asw-mic:hover{color:var(--accent)}
    .asw-mic:active{box-shadow:var(--in-sm)}
    .asw-mic[hidden]{display:none}
    .asw-mic svg{width:16px;height:16px}
    .asw-input-wrap.asw-listening .asw-mic{color:var(--err);box-shadow:var(--in-sm)}

    /* send — the only accent-filled control */
    .asw-send{width:40px;height:40px;flex:none;border:none;border-radius:var(--r-pill);display:grid;place-items:center;
      background:var(--accent);color:var(--on-accent);cursor:pointer;
      box-shadow:var(--x-accent);
      transition:box-shadow var(--ms) var(--ease),background var(--ms) var(--ease),transform var(--ms) var(--ease)}
    .asw-send:hover{background:color-mix(in srgb,var(--accent) 88%,#000)}
    .asw-send:active{box-shadow:var(--in-sm);transform:scale(.96)}
    .asw-send:disabled{cursor:default;background:var(--surface);color:var(--ink-3);box-shadow:var(--in-sm);transform:none}
    .asw-send svg{width:17px;height:17px}
    .asw-send.asw-stop{background:var(--err);
      box-shadow:-4px -4px 10px color-mix(in srgb,var(--err) 20%,var(--light)),5px 5px 12px color-mix(in srgb,var(--err) 40%,var(--dark))}

    .asw-scrollbtn{position:absolute;top:-48px;inset-inline-end:var(--sp-5);z-index:5;width:36px;height:36px;
      border-radius:var(--r-pill);border:none;background:var(--surface);color:var(--ink-2);
      display:grid;place-items:center;cursor:pointer;box-shadow:var(--x-sm);
      opacity:0;transform:translateY(6px) scale(.9);pointer-events:none;
      transition:opacity var(--ms) var(--ease),transform var(--mm) var(--ease-out),box-shadow var(--ms) var(--ease)}
    .asw-scrollbtn.show{opacity:1;transform:translateY(0) scale(1);pointer-events:auto}
    .asw-scrollbtn:hover{color:var(--accent)}
    .asw-scrollbtn:active{box-shadow:var(--in-sm)}
    ...
    .asw-toast svg{width:14px;height:14px;flex:none;color:var(--warn)}

    /* ═══════════════════════════════════════════════════════════════════
       CITATIONS — pressed tabs
       ═══════════════════════════════════════════════════════════════════ */
    .asw-citations{display:flex;flex-wrap:wrap;align-items:center;gap:var(--sp-2);margin-top:var(--sp-3);
      padding-top:var(--sp-3);border-top:1px solid color-mix(in srgb,var(--dark) 35%,transparent)}
    ...
    .asw-citation{display:inline-flex;align-items:center;gap:var(--sp-1);height:26px;font-size:var(--t-xs);
      font-weight:var(--w-med);color:var(--ink-2);background:var(--surface);border:none;
      ...
      transition:color var(--ms) var(--ease)}
    .asw-citation:hover{color:var(--accent)}
    ...
    .asw-handoff-btn{display:inline-flex;align-items:center;gap:var(--sp-1);height:30px;padding:0 var(--sp-4);
      border:none;border-radius:var(--r-pill);background:var(--surface);color:var(--ink);
      box-shadow:var(--x-sm);
      font-family:inherit;font-size:var(--t-xs);font-weight:var(--w-med);cursor:pointer;text-decoration:none;
      transition:box-shadow var(--ms) var(--ease),color var(--ms) var(--ease)}
    .asw-handoff-btn:hover{color:var(--accent)}
    .asw-handoff-btn:active{box-shadow:var(--in-sm)}
    ...
    .asw-rule-hint{margin-top:var(--sp-2);padding:var(--sp-2) var(--sp-3);
      background:var(--surface);border:none;border-radius:var(--r-sm);box-shadow:var(--in-sm);
    ...
    .asw-messages > .asw-rule-hint{align-self:center;margin:0;padding:var(--sp-1) var(--sp-4);
      border-radius:var(--r-pill);font-size:var(--t-xs);color:var(--ink-3);animation:nu-fade var(--mm) both}
    ...
    .asw-sheet::before{content:"";position:absolute;inset:0;background:color-mix(in srgb,var(--dark) 55%,transparent)}
    ...
    .asw-sheet-grip{width:44px;height:6px;border-radius:var(--r-pill);background:var(--surface);
      box-shadow:var(--in-sm);margin:var(--sp-1) auto var(--sp-5)}
    .asw-sheet-close{position:absolute;top:var(--sp-4);inset-inline-end:var(--sp-5);width:32px;height:32px;
      border:none;border-radius:var(--r-pill);background:var(--surface);color:var(--ink-2);
      display:grid;place-items:center;cursor:pointer;box-shadow:var(--x-sm);
      transition:box-shadow var(--ms) var(--ease),color var(--ms) var(--ease)}
    .asw-sheet-close:hover{color:var(--err)}
    .asw-sheet-close:active{box-shadow:var(--in-sm)}
    ...
    .asw-chan{position:relative;display:flex;align-items:center;gap:var(--sp-3);padding:var(--sp-3) var(--sp-4);
      border:none;border-radius:var(--r-md);background:var(--surface);text-decoration:none;
      box-shadow:var(--x-sm);
      transition:box-shadow var(--ms) var(--ease)}
    .asw-chan:hover{box-shadow:var(--x-md)}
    .asw-chan:active{box-shadow:var(--in-sm)}
    .asw-chan-ic{width:36px;height:36px;flex:none;border-radius:var(--r-pill);display:grid;place-items:center;
      color:var(--chan,var(--accent));background:var(--surface);box-shadow:var(--in-sm)}
    ...
    /* ═══════════════════════════════════════════════════════════════════
       FORM — every field is a pressed groove
       ═══════════════════════════════════════════════════════════════════ */
    ...
    .asw-flabel{display:flex;align-items:center;gap:var(--sp-1);font-size:var(--t-xs);font-weight:var(--w-bold);
      color:var(--ink-2);margin-bottom:var(--sp-2);padding-inline-start:var(--sp-2)}
    ...
    .asw-form input,.asw-form textarea{width:100%;font-family:inherit;font-size:var(--t-md);color:var(--ink);
      padding:var(--sp-3) var(--sp-4);border-radius:var(--r-md);border:none;
      background:var(--surface);box-shadow:var(--in-sm);
      outline:none;font-weight:var(--w-reg);
      transition:box-shadow var(--ms) var(--ease)}
    .asw-form textarea{resize:vertical;min-height:70px;max-height:150px;line-height:1.7}
    .asw-form input::placeholder,.asw-form textarea::placeholder{color:var(--ink-3)}
    .asw-form input:focus,.asw-form textarea:focus{box-shadow:var(--in-md)}
    .asw-field.invalid input,.asw-field.invalid textarea{box-shadow:var(--in-sm),0 0 0 2px color-mix(in srgb,var(--err) 45%,transparent)}
    .asw-field.ok input{box-shadow:var(--in-sm),0 0 0 2px color-mix(in srgb,var(--ok) 40%,transparent)}
    ...
    .asw-form-submit{width:100%;display:inline-flex;align-items:center;justify-content:center;gap:var(--sp-2);
      margin-top:var(--sp-3);height:48px;padding:0 var(--sp-6);
      border:none;border-radius:var(--r-pill);
      background:var(--accent);color:var(--on-accent);
      box-shadow:var(--x-accent);
      font-family:inherit;font-size:var(--t-md);font-weight:var(--w-bold);cursor:pointer;
      transition:box-shadow var(--ms) var(--ease),background var(--ms) var(--ease)}
    .asw-form-submit:hover{background:color-mix(in srgb,var(--accent) 88%,#000)}
    .asw-form-submit:active{box-shadow:var(--in-sm)}
    .asw-form-submit:disabled{background:var(--surface);color:var(--ink-3);cursor:default;box-shadow:var(--in-sm)}
    ...
    .asw-spinner{width:15px;height:15px;flex:none;border:2px solid color-mix(in srgb,currentColor 28%,transparent);
      border-top-color:currentColor;border-radius:var(--r-pill);animation:nu-spin .8s linear infinite}
    ...
    .asw-form-success{display:flex;flex-direction:column;align-items:center;text-align:center;gap:var(--sp-3);
      padding:var(--sp-6) var(--sp-2) var(--sp-3);animation:nu-fade var(--mm) var(--ease-out) both}
    .asw-checkmark{width:60px;height:60px;stroke:var(--ok);stroke-width:3;fill:none;
      stroke-linecap:round;stroke-linejoin:round}
    .asw-checkmark circle{stroke:color-mix(in srgb,var(--dark) 70%,transparent);
      stroke-dasharray:170;stroke-dashoffset:170;animation:nu-draw 700ms var(--ease-out) 60ms forwards}
    .asw-checkmark path{stroke-dasharray:44;stroke-dashoffset:44;animation:nu-draw 400ms var(--ease-out) 600ms forwards}
    ...
    .asw-fs-close{margin-top:var(--sp-4);height:38px;padding:0 var(--sp-6);border:none;border-radius:var(--r-pill);
      background:var(--surface);color:var(--ink);box-shadow:var(--x-sm);
      font-family:inherit;font-size:var(--t-sm);font-weight:var(--w-bold);cursor:pointer;
      transition:box-shadow var(--ms) var(--ease)}
    .asw-fs-close:active{box-shadow:var(--in-sm)}
    .asw-leadform-wrap{animation:nu-fade var(--mm) var(--ease-out) both}
    .asw-leadform{position:relative;flex:1 1 100%;margin:var(--sp-2) 0;padding:var(--sp-4);
      border:none;border-radius:var(--r-md);background:var(--surface);box-shadow:var(--in-sm);
      animation:nu-fade var(--mm) var(--ease-out) both}

    /* ═══════════════════════════════════════════════════════════════════
       MOTION — soft, no overshoot
       ═══════════════════════════════════════════════════════════════════ */
    @keyframes nu-fade{from{opacity:0}to{opacity:1}}
    @keyframes nu-rise{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
    @keyframes nu-in{from{opacity:0;transform:scale(.7)}to{opacity:1;transform:scale(1)}}
    @keyframes nu-pop{from{transform:scale(.4)}to{transform:scale(1)}}
    @keyframes nu-spin{to{transform:rotate(360deg)}}
    @keyframes nu-ping{0%{box-shadow:0 0 0 0 color-mix(in srgb,var(--accent) 35%,transparent);opacity:.9}
                       70%,100%{box-shadow:0 0 0 16px transparent;opacity:0}}
    @keyframes nu-bounce{0%,100%{transform:translateY(0);opacity:.5}50%{transform:translateY(-4px);opacity:1}}
    @keyframes nu-blink{50%{opacity:0}}
    @keyframes nu-draw{to{stroke-dashoffset:0}}
    @keyframes nu-wave{0%,100%{transform:scaleY(.3)}50%{transform:scaleY(1)}}

    /* ═══════════════════════════════════════════════════════════════════
       RESPONSIVE / PREFERENCES / A11Y
       ═══════════════════════════════════════════════════════════════════ */
    @media(max-width:480px){
      .asw-fab{width:58px;height:58px}
      ...
      .asw-root.mobile-fullscreen .asw-panel{position:fixed!important;inset:0!important;width:100vw!important;
        ...
        border-radius:0!important;box-shadow:none!important;transform:translateY(16px)!important}
    ...
    /* focus — a soft accent halo, never a hard ring */
    .asw :focus-visible{outline:none;box-shadow:var(--x-sm),0 0 0 3px color-mix(in srgb,var(--accent) 35%,transparent)}
    .asw ::selection{background:color-mix(in srgb,var(--accent) 25%,transparent)}
