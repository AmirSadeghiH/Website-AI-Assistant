...
(function () {
  ...
  var CSS = `
    /* ═══════════════════════════════════════════════════════════════════════
       AI Support Widget — GLASSMORPHISM (Frosted Glass)
       ───────────────────────────────────────────────────────────────────────
       Every surface is a pane of frosted glass. The recipe, applied
       consistently to every layer:

         1. TRANSLUCENCY  · a low-alpha white fill, never an opaque colour
         2. BACKDROP BLUR · backdrop-filter: blur() + saturate() so whatever
                            sits behind bleeds through, softened and vivid
         3. LIGHT EDGE    · a 1px hairline border, brighter on the top edge
                            where light would catch the bevel
         4. INNER SHEEN   · inset highlight along the top, a faint inset
                            shadow along the bottom — the thickness of glass
         5. SOFT SHADOW   · a wide, low-opacity drop shadow so the pane
                            floats above the backdrop

       Depth is expressed as GLASS THICKNESS. Panes stack in tiers, each
       blurring the tier beneath it:
            tier 1  panel        blur 40px
            tier 2  header/footer/bubbles  blur 20px
            tier 3  chips, buttons          blur 12px
            tier 4  flyouts, sheet          blur 60px

       Because glass needs something to refract, the panel carries its own
       coloured light sources (--asw-glow and two orbs) so the effect never
       collapses on a plain white host page.
    ...
    :host{all:initial;--asw-font:'Vazirmatn','Vazir','IRANSans',ui-sans-serif,system-ui,-apple-system,sans-serif}
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}

    /* ═══════════════════════════════════════════════════════════════════
       TOKENS — the glass recipe
       ═══════════════════════════════════════════════════════════════════ */
    .asw{
      --asw-primary:#7b5cff;--asw-secondary:#3fa9ff;--asw-accent:#ff5c9d;
      --accent:var(--asw-primary);
      --accent-2:var(--asw-secondary);

      /* glass fills — white at low alpha */
      --g-1:rgba(255,255,255,.14);
      --g-2:rgba(255,255,255,.22);
      --g-3:rgba(255,255,255,.32);
      --g-4:rgba(255,255,255,.44);
      --g-hover:rgba(255,255,255,.3);
      --g-press:rgba(255,255,255,.12);

      /* the pane behind everything */
      --pane:rgba(255,255,255,.16);
      --pane-solid:rgba(246,247,252,.86);

      /* edges — light catches the top bevel */
      --edge:rgba(255,255,255,.5);
      --edge-soft:rgba(255,255,255,.28);
      --edge-dim:rgba(255,255,255,.16);

      /* the thickness of the glass */
      --sheen:inset 0 1px 0 rgba(255,255,255,.55), inset 0 -1px 0 rgba(255,255,255,.1);
      --sheen-sm:inset 0 1px 0 rgba(255,255,255,.4);

      /* blur tiers */
      --blur-1:blur(40px) saturate(180%);
      --blur-2:blur(20px) saturate(170%);
      --blur-3:blur(12px) saturate(160%);
      --blur-4:blur(60px) saturate(190%);

      /* floating shadows — wide and soft */
      --sh-sm:0 2px 8px rgba(16,18,40,.08);
      --sh-md:0 8px 24px rgba(16,18,40,.12);
      --sh-lg:0 16px 40px rgba(16,18,40,.16);
      --sh-xl:0 24px 70px rgba(16,18,40,.28), 0 6px 20px rgba(16,18,40,.14);

      /* ink — dark enough to stay legible on frosted glass */
      --ink:#1c1e2e;
      --ink-2:#4d5170;
      --ink-3:#7a7f9c;
      --on-accent:#ffffff;
      ...
      /* status */
      --ok:#0f9d6b;--err:#e0365e;--warn:#c9821f;--online:#22c98a;

      /* geometry — glass wants soft, continuous curves */
      --r-sm:10px;--r-md:16px;--r-lg:22px;--r-xl:28px;--r-pill:999px;

      --sp-1:4px;--sp-2:8px;--sp-3:12px;--sp-4:16px;--sp-5:20px;--sp-6:24px;--sp-8:32px;--sp-10:40px;

      --t-xs:11px;--t-sm:12.5px;--t-md:14px;--t-lg:16px;--t-xl:20px;
      --w-reg:450;--w-med:550;--w-bold:700;

      --ms:180ms;--mm:300ms;--ml:460ms;
      --ease:cubic-bezier(.4,0,.2,1);
      --ease-out:cubic-bezier(.16,1,.3,1);

      /* aliases consumed by the unchanged JS */
      --asw-bg:var(--pane);--asw-surface:var(--g-2);--asw-surface-2:var(--g-1);
      --asw-text:var(--ink);--asw-text-secondary:var(--ink-2);--asw-text-muted:var(--ink-3);
      --asw-border:var(--edge-soft);--asw-border-strong:var(--edge);
      --asw-bot-bg:var(--g-3);--asw-bot-text:var(--ink);
      --asw-user-bg:var(--accent);--asw-user-bg-2:var(--accent-2);--asw-user-text:var(--on-accent);
      --asw-input-bg:var(--g-2);--asw-input-focus-bg:var(--g-3);
      --asw-shadow:var(--sh-xl);--asw-header-bg:transparent;
    ...
    /* ─── smoked glass ─── */
    .asw.dark{
      --g-1:rgba(255,255,255,.05);
      --g-2:rgba(255,255,255,.08);
      --g-3:rgba(255,255,255,.12);
      --g-4:rgba(255,255,255,.18);
      --g-hover:rgba(255,255,255,.14);
      --g-press:rgba(255,255,255,.04);

      --pane:rgba(22,24,42,.5);
      --pane-solid:rgba(24,26,44,.9);

      --edge:rgba(255,255,255,.18);
      --edge-soft:rgba(255,255,255,.1);
      --edge-dim:rgba(255,255,255,.06);

      --sheen:inset 0 1px 0 rgba(255,255,255,.14), inset 0 -1px 0 rgba(0,0,0,.2);
      --sheen-sm:inset 0 1px 0 rgba(255,255,255,.1);
      ...
      --sh-lg:0 16px 40px rgba(0,0,0,.46);
      --sh-xl:0 24px 70px rgba(0,0,0,.6), 0 6px 20px rgba(0,0,0,.4);

      --ok:#3ddba4;--err:#ff7a95;--warn:#f0b959;--online:#3ddba4;
    }

    /* ═══════════════════════════════════════════════════════════════════
       ROOT + the coloured light the glass refracts
       ═══════════════════════════════════════════════════════════════════ */
    ...
    .asw-glow{position:fixed;z-index:0;width:340px;height:340px;border-radius:var(--r-pill);pointer-events:none;
      background:radial-gradient(circle at 30% 30%,color-mix(in srgb,var(--accent) 70%,transparent),transparent 62%),
                 radial-gradient(circle at 70% 70%,color-mix(in srgb,var(--accent-2) 60%,transparent),transparent 60%);
      filter:blur(70px);opacity:0;transform:scale(.7);
      transition:opacity var(--ml) var(--ease),transform var(--ml) var(--ease-out)}
    .asw-glow.show{opacity:.75;transform:scale(1)}

    /* ═══════════════════════════════════════════════════════════════════
       LAUNCHER — a glass lens over an accent core
       ═══════════════════════════════════════════════════════════════════ */
    .asw-fab{position:fixed;width:60px;height:60px;border:1px solid var(--edge);border-radius:var(--r-pill);
      cursor:pointer;z-index:2;display:grid;place-items:center;overflow:hidden;
      background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 92%,transparent),color-mix(in srgb,var(--accent-2) 88%,transparent));
      color:#fff;
      box-shadow:var(--sh-lg),var(--sheen);
      transition:transform var(--mm) var(--ease-out),box-shadow var(--mm) var(--ease);
      animation:gl-in var(--ml) var(--ease-out) 100ms both}
    /* the specular streak across the lens */
    .asw-fab::after{content:"";position:absolute;top:-60%;left:-10%;width:70%;height:170%;pointer-events:none;
      background:linear-gradient(90deg,transparent,rgba(255,255,255,.5),transparent);
      transform:rotate(22deg) translateX(-120%);
      transition:transform var(--ml) var(--ease)}
    .asw-fab:hover::after{transform:rotate(22deg) translateX(220%)}
    .asw-fab:hover{transform:translateY(-2px);box-shadow:var(--sh-xl),var(--sheen)}
    .asw-fab:active{transform:translateY(0) scale(.97)}
    .asw-fab.active{background:var(--g-3);color:var(--ink);
      backdrop-filter:var(--blur-2);-webkit-backdrop-filter:var(--blur-2);
      border-color:var(--edge-soft)}
    .asw-fab svg{width:25px;height:25px;position:relative;z-index:1}
    .asw-fab-shine{position:absolute;inset:0;border-radius:inherit;pointer-events:none;
      background:radial-gradient(120% 80% at 50% 0%,rgba(255,255,255,.4),transparent 62%)}
    .asw-fab.active .asw-fab-shine{opacity:.5}
    .asw-fab-icon-main,.asw-fab-icon-close{position:absolute;inset:0;display:grid;place-items:center;
      transition:opacity var(--ms) var(--ease),transform var(--mm) var(--ease-out)}
    ...
    .asw-fab-custom-icon{width:28px;height:28px;border-radius:var(--r-sm);object-fit:contain;position:relative;z-index:1}
    .asw-fab-pulse{position:absolute;inset:0;border-radius:inherit;border:1.5px solid rgba(255,255,255,.6);
      opacity:0;animation:gl-ping 2.8s var(--ease) 1.4s infinite;pointer-events:none}
    .asw-fab.active .asw-fab-pulse{display:none}

    .asw-fab-badge{position:absolute;top:-1px;inset-inline-end:-1px;min-width:21px;height:21px;padding:0 6px;
      ...
      border:1.5px solid rgba(255,255,255,.6);box-shadow:var(--sh-sm);z-index:2;pointer-events:none}
    .asw-fab-badge[hidden]{display:none}
    .asw-fab-badge.asw-pop{animation:gl-pop var(--mm) var(--ease-out)}

    /* tooltip — a small frosted chip */
    .asw-fab-label{position:absolute;inset-inline-end:calc(100% + var(--sp-3));top:50%;
      transform:translateY(-50%) translateX(6px);white-space:nowrap;
      padding:var(--sp-2) var(--sp-4);border-radius:var(--r-pill);
      background:var(--pane);border:1px solid var(--edge-soft);color:var(--ink);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      box-shadow:var(--sh-md),var(--sheen-sm);
    ...
    /* ═══════════════════════════════════════════════════════════════════
       TEASER — a floating pane
       ═══════════════════════════════════════════════════════════════════ */
    .asw-teaser{position:fixed;z-index:1;max-width:270px;display:flex;align-items:flex-start;gap:var(--sp-3);
      padding:var(--sp-4);border-radius:var(--r-lg);
      background:var(--pane);border:1px solid var(--edge-soft);
      backdrop-filter:var(--blur-2);-webkit-backdrop-filter:var(--blur-2);
      box-shadow:var(--sh-lg),var(--sheen);
      cursor:pointer;opacity:0;transform:translateY(10px) scale(.98);
      transition:opacity var(--mm) var(--ease),transform var(--mm) var(--ease-out);
      pointer-events:none}
    @supports not ((backdrop-filter:blur(1px)) or (-webkit-backdrop-filter:blur(1px))){
      .asw-teaser{background:var(--pane-solid)}
    }
    .asw-teaser.show{opacity:1;transform:translateY(0) scale(1);pointer-events:auto}
    .asw-teaser-orb{width:36px;height:36px;flex:none;border-radius:var(--r-md);display:grid;place-items:center;
      background:linear-gradient(135deg,var(--accent),var(--accent-2));color:#fff;
      box-shadow:var(--sh-sm),var(--sheen-sm)}
    ...
    .asw-teaser-close{position:absolute;top:var(--sp-2);inset-inline-start:var(--sp-2);width:26px;height:26px;
      border:1px solid transparent;border-radius:var(--r-pill);background:transparent;color:var(--ink-3);
      display:grid;place-items:center;cursor:pointer;opacity:0;
      transition:opacity var(--ms) var(--ease),background var(--ms) var(--ease)}
    .asw-teaser:hover .asw-teaser-close,.asw-teaser-close:focus-visible{opacity:1}
    .asw-teaser-close:hover{background:var(--g-3);border-color:var(--edge-dim);color:var(--ink)}
    .asw-teaser-close svg{width:10px;height:10px}
    @media(max-width:520px){.asw-teaser{display:none}}

    /* ═══════════════════════════════════════════════════════════════════
       PANEL — the primary pane; carries its own light sources
       ═══════════════════════════════════════════════════════════════════ */
    .asw-panel{position:fixed;z-index:3;width:var(--asw-panel-w);height:var(--asw-panel-h);
      ...
      box-shadow:var(--sh-xl),var(--sheen);
    ...
    @supports not ((backdrop-filter:blur(1px)) or (-webkit-backdrop-filter:blur(1px))){
      .asw-panel{background:var(--pane-solid)}
    }
    /* two coloured orbs drifting behind the glass, so there is always
       something for the blur to refract */
    .asw-panel::before,.asw-panel::after{content:"";position:absolute;border-radius:var(--r-pill);
      pointer-events:none;z-index:-1;filter:blur(50px)}
    .asw-panel::before{width:280px;height:280px;top:-90px;inset-inline-end:-70px;
      background:radial-gradient(circle,color-mix(in srgb,var(--accent) 55%,transparent),transparent 70%);
      animation:gl-drift-a 18s var(--ease) infinite alternate}
    .asw-panel::after{width:240px;height:240px;bottom:-70px;inset-inline-start:-60px;
      background:radial-gradient(circle,color-mix(in srgb,var(--accent-2) 50%,transparent),transparent 70%);
      animation:gl-drift-b 22s var(--ease) infinite alternate}
    .asw-panel.open{opacity:1;transform:translateY(0) scale(1);pointer-events:auto;visibility:visible;
      transition:opacity var(--mm) var(--ease),transform var(--ml) var(--ease-out),visibility 0s}
    ...
    .asw-root.corner-tr .asw-panel,.asw-root.corner-tl .asw-panel{transform:translateY(-16px) scale(.96)}
    .asw-root.corner-tr .asw-panel.open,.asw-root.corner-tl .asw-panel.open{transform:translateY(0) scale(1)}

    /* ═══════════════════════════════════════════════════════════════════
       HEADER — a thinner pane laid over the panel
       ═══════════════════════════════════════════════════════════════════ */
    ...
    .asw-avatar{width:40px;height:40px;border-radius:var(--r-pill);display:grid;place-items:center;overflow:hidden;
      background:linear-gradient(135deg,var(--accent),var(--accent-2));color:#fff;
      font-size:var(--t-md);font-weight:var(--w-bold);
      border:1px solid var(--edge);box-shadow:var(--sh-sm),var(--sheen-sm)}
    .asw-avatar img{width:100%;height:100%;border-radius:inherit;object-fit:cover}
    .asw-status{position:absolute;bottom:0;inset-inline-end:0;width:12px;height:12px;border-radius:var(--r-pill);
      background:var(--online);border:2px solid rgba(255,255,255,.7);
      box-shadow:0 0 8px color-mix(in srgb,var(--online) 70%,transparent)}
    ...
    /* chips — the thinnest glass */
    .asw-ai-chip,.asw-badge{display:inline-flex;align-items:center;gap:var(--sp-1);flex:none;height:22px;
      padding:0 var(--sp-2);border-radius:var(--r-pill);
      background:var(--g-2);border:1px solid var(--edge-dim);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      font-size:10px;font-weight:var(--w-bold);letter-spacing:.03em;white-space:nowrap;color:var(--ink-2)}
    .asw-ai-chip{color:var(--accent);border-color:color-mix(in srgb,var(--accent) 30%,transparent);
      background:color-mix(in srgb,var(--accent) 12%,var(--g-2))}
    .asw.dark .asw-ai-chip{color:color-mix(in srgb,var(--accent) 60%,#fff)}
    .asw-ai-chip svg{width:10px;height:10px}
    .asw-badge-dot{width:6px;height:6px;border-radius:var(--r-pill);background:var(--online);flex:none;
      box-shadow:0 0 6px color-mix(in srgb,var(--online) 80%,transparent)}

    .asw-actions{position:relative;display:flex;gap:var(--sp-1);flex:none}
    .asw-header-btn{width:34px;height:34px;border:1px solid transparent;border-radius:var(--r-pill);
      background:transparent;color:var(--ink-2);display:grid;place-items:center;cursor:pointer;
      transition:background var(--ms) var(--ease),color var(--ms) var(--ease),border-color var(--ms) var(--ease)}
    .asw-header-btn:hover{background:var(--g-hover);border-color:var(--edge-soft);color:var(--ink);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3)}
    .asw-header-btn:active{background:var(--g-press)}
    .asw-header-btn svg{width:15px;height:15px;transition:transform var(--ml) var(--ease-out)}
    ...
    .asw-netstatus{position:relative;z-index:3;flex:none;display:flex;align-items:center;justify-content:center;
      ...
      background:color-mix(in srgb,var(--warn) 16%,var(--g-2));
      border-bottom:1px solid var(--edge-dim);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      color:var(--warn);font-size:var(--t-xs);font-weight:var(--w-bold)}
    .asw-netstatus[hidden]{display:none}
    .asw-netstatus svg{width:13px;height:13px;flex:none}

    /* ═══════════════════════════════════════════════════════════════════
       MESSAGES — transparent, so the orbs show through
       ═══════════════════════════════════════════════════════════════════ */
    .asw-messages{position:relative;z-index:1;flex:1;min-height:0;overflow-y:auto;overflow-x:hidden;
      overscroll-behavior:contain;padding:var(--sp-5);display:flex;flex-direction:column;gap:var(--sp-3);
      background:transparent;
      scroll-behavior:smooth;scrollbar-width:thin;scrollbar-color:var(--edge) transparent}
    .asw-messages::-webkit-scrollbar{width:8px}
    .asw-messages::-webkit-scrollbar-track{background:transparent}
    .asw-messages::-webkit-scrollbar-thumb{border-radius:var(--r-pill);background:rgba(255,255,255,.4);
      border:2px solid transparent;background-clip:padding-box}
    .asw.dark .asw-messages::-webkit-scrollbar-thumb{background:rgba(255,255,255,.16);background-clip:padding-box}

    .asw-row{position:relative;z-index:1;display:flex;flex-wrap:wrap;align-items:flex-end;gap:var(--sp-2);
      max-width:88%;animation:gl-rise var(--ml) var(--ease-out) both}
    ...
    .asw-msg-avatar{width:30px;height:30px;flex:0 0 30px;border-radius:var(--r-pill);display:grid;place-items:center;
      font-size:10px;font-weight:var(--w-bold);overflow:hidden;border:1px solid var(--edge-soft)}
    .asw-row.bot .asw-msg-avatar{background:linear-gradient(135deg,var(--accent),var(--accent-2));color:#fff;
      box-shadow:var(--sheen-sm)}
    .asw-row.bot .asw-msg-avatar img{width:100%;height:100%;object-fit:cover}
    .asw-row.user .asw-msg-avatar{background:var(--g-3);color:var(--ink-2);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3)}
    ...
    .asw-divider{position:relative;z-index:1;align-self:center;display:flex;align-items:center;gap:var(--sp-2);
      margin:var(--sp-1) 0;padding:3px var(--sp-3);border-radius:var(--r-pill);
      background:var(--g-2);border:1px solid var(--edge-dim);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      color:var(--ink-3);font-size:10px;font-weight:var(--w-bold);letter-spacing:.03em;
      animation:gl-fade var(--mm) var(--ease-out) both}

    /* ── BUBBLES ──
       bot  : a frosted pane
       user : tinted glass — the accent gradient seen through the same pane */
    ...
    .asw-row.bot .asw-bubble{background:var(--g-3);color:var(--ink);
      border:1px solid var(--edge-soft);
      border-radius:var(--r-md) var(--r-md) var(--sp-1) var(--r-md);
      backdrop-filter:var(--blur-2);-webkit-backdrop-filter:var(--blur-2);
      box-shadow:var(--sh-sm),var(--sheen-sm)}
    .asw-row.user .asw-bubble{
      background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 88%,transparent),color-mix(in srgb,var(--accent-2) 82%,transparent));
      color:var(--on-accent);
      border:1px solid rgba(255,255,255,.35);
      border-radius:var(--r-md) var(--r-md) var(--r-md) var(--sp-1);
      backdrop-filter:var(--blur-2);-webkit-backdrop-filter:var(--blur-2);
      box-shadow:var(--sh-md),inset 0 1px 0 rgba(255,255,255,.4)}
    ...
    .asw-bubble.asw-error-bubble{background:color-mix(in srgb,var(--err) 16%,var(--g-2));color:var(--ink);
    ...
    .asw-bubble a{color:var(--accent);font-weight:var(--w-med);text-decoration:none;
      border-bottom:1px solid color-mix(in srgb,var(--accent) 40%,transparent)}
    .asw.dark .asw-bubble a{color:color-mix(in srgb,var(--accent) 55%,#fff)}
    .asw-row.user .asw-bubble a{color:#fff;border-bottom-color:rgba(255,255,255,.5)}
    ...
    .asw-row.user .asw-bubble li::marker{color:rgba(255,255,255,.8)}
    .asw-bubble p{margin:var(--sp-1) 0}
    .asw-bubble p:first-child{margin-top:0}
    .asw-bubble p:last-child{margin-bottom:0}
    .asw-bubble code:not(.asw-code-block code){padding:1px 6px;border-radius:6px;
      background:var(--g-2);border:1px solid var(--edge-dim);
      font-family:ui-monospace,'JetBrains Mono',Consolas,monospace;font-size:.87em;color:inherit;white-space:pre-wrap}
    .asw-row.user .asw-bubble code{background:rgba(255,255,255,.2);border-color:rgba(255,255,255,.3)}

    /* table — a glass sheet */
    .asw-table-wrap{margin:var(--sp-3) 0;border:1px solid var(--edge-soft);border-radius:var(--r-md);
      overflow:hidden;overflow-x:auto;background:var(--g-2);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);direction:rtl}
    .asw-table-wrap table{border-collapse:collapse;width:100%;font-size:var(--t-sm);line-height:1.7}
    .asw-table-wrap th{background:var(--g-2);font-weight:var(--w-bold);text-align:start;
      padding:var(--sp-2) var(--sp-3);white-space:nowrap;color:var(--ink);
      border-bottom:1px solid var(--edge-soft)}
    .asw-table-wrap td{padding:var(--sp-2) var(--sp-3);color:var(--ink-2);border-top:1px solid var(--edge-dim)}
    .asw-table-wrap tbody tr:hover{background:var(--g-1)}
    .asw-table-wrap th[style*="center"],.asw-table-wrap td[style*="center"]{text-align:center}

    /* code — smoked glass over a dark tint */
    .asw-code-wrap{position:relative;margin:var(--sp-3) 0;border-radius:var(--r-md);overflow:hidden;
      background:rgba(18,20,38,.6);border:1px solid var(--edge-dim);
      backdrop-filter:var(--blur-2);-webkit-backdrop-filter:var(--blur-2);
      box-shadow:var(--sheen-sm);direction:ltr;text-align:left}
    ...
    .asw-code-dots span{width:9px;height:9px;border-radius:var(--r-pill);background:rgba(255,255,255,.2)}
    .asw-code-lang{font-size:10px;color:rgba(255,255,255,.5);font-weight:var(--w-bold);letter-spacing:.1em;
      text-transform:uppercase;margin-left:var(--sp-1)}
    .asw-code-copy{margin-left:auto;display:inline-flex;align-items:center;gap:var(--sp-1);height:24px;
      padding:0 var(--sp-2);border:1px solid rgba(255,255,255,.14);border-radius:var(--r-pill);
    ...
    .asw-code-copy.copied{color:var(--ok)}
    ...
    .asw-code-block{margin:0;padding:var(--sp-3) var(--sp-4);overflow-x:auto;
      ...
      color:#e7e9ff;white-space:pre;tab-size:2;background:none}
    ...
    .asw-time{padding:var(--sp-1) var(--sp-1) 0;font-size:10px;color:var(--ink-3);font-weight:var(--w-med);
    ...
    .asw-fb-btn{width:28px;height:28px;border:1px solid transparent;border-radius:var(--r-pill);
      background:transparent;color:var(--ink-3);cursor:pointer;display:grid;place-items:center;
      transition:background var(--ms) var(--ease),color var(--ms) var(--ease),border-color var(--ms) var(--ease)}
    .asw-fb-btn:hover{background:var(--g-hover);border-color:var(--edge-dim);color:var(--ink)}
    .asw-fb-btn.active{background:color-mix(in srgb,var(--accent) 18%,transparent);
      border-color:color-mix(in srgb,var(--accent) 35%,transparent);color:var(--accent)}
    .asw-fb-btn.active.thumbs-up,.asw-fb-btn.copied{color:var(--ok);
      background:color-mix(in srgb,var(--ok) 16%,transparent);border-color:color-mix(in srgb,var(--ok) 32%,transparent)}
    .asw-fb-btn.active.thumbs-down{color:var(--err);
      background:color-mix(in srgb,var(--err) 16%,transparent);border-color:color-mix(in srgb,var(--err) 32%,transparent)}
    .asw-fb-btn svg{width:13px;height:13px}

    .asw-retry{display:inline-flex;align-items:center;justify-content:center;gap:var(--sp-2);margin-top:var(--sp-2);
      height:30px;padding:0 var(--sp-4);border:1px solid color-mix(in srgb,var(--err) 40%,transparent);
      ...
      transition:background var(--ms) var(--ease)}
    .asw-retry:hover{background:color-mix(in srgb,var(--err) 22%,transparent)}
    ...
    .asw-typing-ava{width:30px;height:30px;flex:none;border-radius:var(--r-pill);
      background:linear-gradient(135deg,var(--accent),var(--accent-2));color:#fff;display:grid;place-items:center;
      border:1px solid var(--edge-soft);box-shadow:var(--sheen-sm)}
    ...
    .asw-typing-bubble{display:flex;align-items:center;gap:5px;padding:var(--sp-3) var(--sp-4);
      background:var(--g-3);border:1px solid var(--edge-soft);border-radius:var(--r-md);
      backdrop-filter:var(--blur-2);-webkit-backdrop-filter:var(--blur-2);
      box-shadow:var(--sh-sm),var(--sheen-sm)}
    .asw-typing-dot{width:6px;height:6px;border-radius:var(--r-pill);background:var(--accent);
      animation:gl-bounce 1.2s var(--ease) infinite}
    ...
    .asw-bubble.streaming::after{content:"";display:inline-block;width:2px;height:15px;margin-inline-start:3px;
      border-radius:var(--r-pill);background:currentColor;vertical-align:-2px;animation:gl-blink 1s steps(2) infinite}

    /* ═══════════════════════════════════════════════════════════════════
       HERO
       ═══════════════════════════════════════════════════════════════════ */
    .asw-hero{position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;text-align:center;
      gap:var(--sp-4);padding:var(--sp-8) var(--sp-3) var(--sp-4);
      transition:opacity var(--mm) var(--ease),transform var(--mm) var(--ease)}
    .asw-hero>*{animation:gl-rise var(--ml) var(--ease-out) both}
    ...
    .asw-hero-spot{position:absolute;top:-10px;left:50%;transform:translateX(-50%);width:300px;height:180px;
    ...
    .asw-hero-orb{position:relative;width:80px;height:80px;border-radius:var(--r-pill);display:grid;place-items:center;
      background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 90%,transparent),color-mix(in srgb,var(--accent-2) 85%,transparent));
      color:#fff;border:1px solid rgba(255,255,255,.45);
      backdrop-filter:var(--blur-2);-webkit-backdrop-filter:var(--blur-2);
      box-shadow:var(--sh-lg),inset 0 2px 0 rgba(255,255,255,.5)}
    .asw-hero-orb::after{content:"";position:absolute;top:8%;left:14%;width:52%;height:34%;border-radius:var(--r-pill);
      background:linear-gradient(180deg,rgba(255,255,255,.55),transparent);pointer-events:none}
    .asw-hero-orb svg{width:33px;height:33px;position:relative;z-index:1}
    .asw-hero-orb img{width:100%;height:100%;border-radius:inherit;object-fit:cover}
    .asw-hero-p{position:absolute;border-radius:var(--r-pill);background:var(--g-4);
      border:1px solid var(--edge-soft);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)}
    .asw-hero-p.p1{width:12px;height:12px;top:-12px;inset-inline-start:2px;animation:gl-float 6s var(--ease) infinite}
    .asw-hero-p.p2{width:9px;height:9px;top:18px;inset-inline-end:-20px;animation:gl-float 7.5s var(--ease) .6s infinite}
    .asw-hero-p.p3{width:7px;height:7px;bottom:-6px;inset-inline-start:-18px;animation:gl-float 6.8s var(--ease) 1.2s infinite}
    .asw-hero-text{font-size:var(--t-lg);font-weight:var(--w-med);color:var(--ink);line-height:1.9;max-width:296px}
    ...
    .asw-hero-grid .asw-suggestion{height:auto;justify-content:flex-start;text-align:start;white-space:normal;
      border-radius:var(--r-md);padding:var(--sp-3);line-height:1.6;font-size:var(--t-sm);gap:var(--sp-2)}
    ...
    /* ═══════════════════════════════════════════════════════════════════
       SUGGESTIONS — small panes that brighten on hover
       ═══════════════════════════════════════════════════════════════════ */
    .asw-suggestions{position:relative;z-index:2;flex:none;display:flex;gap:var(--sp-2);align-items:center;
      overflow-x:auto;padding:var(--sp-2) var(--sp-5);background:transparent;
      scrollbar-width:none;-webkit-overflow-scrolling:touch}
    .asw-suggestions::-webkit-scrollbar{display:none}
    .asw-suggestions[hidden]{display:none}
    .asw-suggestion{display:inline-flex;align-items:center;gap:var(--sp-2);flex:0 0 auto;height:34px;
      padding:0 var(--sp-4);border:1px solid var(--edge-soft);border-radius:var(--r-pill);
      background:var(--g-2);color:var(--ink);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      box-shadow:var(--sheen-sm);
      font-family:inherit;font-size:var(--t-sm);font-weight:var(--w-med);cursor:pointer;
      white-space:nowrap;text-decoration:none;
      transition:background var(--ms) var(--ease),transform var(--ms) var(--ease-out),box-shadow var(--ms) var(--ease)}
    .asw-suggestion:hover{background:var(--g-hover);transform:translateY(-1px);box-shadow:var(--sh-sm),var(--sheen-sm)}
    .asw-suggestion:active{transform:translateY(0);background:var(--g-press)}
    ...
    .asw-footer{position:relative;z-index:2;flex:none;
      padding:var(--sp-3) var(--sp-5) calc(var(--sp-4) + env(safe-area-inset-bottom,0px));
      background:var(--g-1);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      border-top:1px solid var(--edge-dim)}
    .asw-input-wrap{position:relative;display:flex;align-items:flex-end;gap:var(--sp-1);padding:var(--sp-1);
      padding-inline:var(--sp-4) var(--sp-1);
      border:1px solid var(--edge-soft);border-radius:var(--r-xl);
      background:var(--g-2);
      backdrop-filter:var(--blur-2);-webkit-backdrop-filter:var(--blur-2);
      box-shadow:var(--sheen-sm);
      transition:background var(--ms) var(--ease),border-color var(--ms) var(--ease),box-shadow var(--mm) var(--ease)}
    .asw-input-wrap:focus-within{background:var(--g-3);border-color:color-mix(in srgb,var(--accent) 45%,transparent);
      box-shadow:0 0 0 3px color-mix(in srgb,var(--accent) 16%,transparent),var(--sheen-sm)}
    .asw-input-wrap.asw-listening{border-color:color-mix(in srgb,var(--err) 55%,transparent);
    ...
    .asw-input{min-width:0;flex:1;border:0;outline:0;background:transparent;color:var(--ink);
      ...
      padding:var(--sp-2) 0;max-height:132px;overflow-y:auto;scrollbar-width:none;font-weight:var(--w-reg)}
    ...
    .asw-wave span{width:3px;height:100%;border-radius:var(--r-pill);background:var(--err);
      transform-origin:center;animation:gl-wave .9s var(--ease) infinite}
    ...
    .asw-mic{width:36px;height:36px;flex:none;border:1px solid transparent;border-radius:var(--r-pill);
      background:transparent;color:var(--ink-3);display:grid;place-items:center;cursor:pointer;
      transition:background var(--ms) var(--ease),color var(--ms) var(--ease),border-color var(--ms) var(--ease)}
    .asw-mic:hover{background:var(--g-hover);border-color:var(--edge-dim);color:var(--ink)}
    .asw-mic[hidden]{display:none}
    .asw-mic svg{width:16px;height:16px}
    .asw-input-wrap.asw-listening .asw-mic{color:var(--err);
      background:color-mix(in srgb,var(--err) 14%,transparent)}

    /* send — a tinted glass lens */
    .asw-send{position:relative;width:38px;height:38px;flex:none;border:1px solid rgba(255,255,255,.4);
      border-radius:var(--r-pill);display:grid;place-items:center;overflow:hidden;
      ...
      transition:transform var(--ms) var(--ease-out),box-shadow var(--ms) var(--ease),opacity var(--ms) var(--ease)}
    .asw-send:hover{transform:translateY(-1px);box-shadow:var(--sh-md),inset 0 1px 0 rgba(255,255,255,.45)}
    .asw-send:active{transform:translateY(0) scale(.95)}
    .asw-send:disabled{cursor:default;background:var(--g-2);color:var(--ink-3);
      border-color:var(--edge-dim);box-shadow:none;transform:none}
    .asw-send svg{width:16px;height:16px;position:relative;z-index:1}
    .asw-send.asw-stop{background:linear-gradient(135deg,var(--err),color-mix(in srgb,var(--err) 70%,#000))}

    .asw-scrollbtn{position:absolute;top:-46px;inset-inline-end:var(--sp-5);z-index:5;width:34px;height:34px;
      border-radius:var(--r-pill);border:1px solid var(--edge-soft);
      background:var(--pane);color:var(--ink-2);display:grid;place-items:center;cursor:pointer;
      backdrop-filter:var(--blur-2);-webkit-backdrop-filter:var(--blur-2);
      box-shadow:var(--sh-md),var(--sheen-sm);
    ...
    .asw-scrollbtn:hover{background:var(--g-hover);color:var(--ink)}
    ...
    .asw-powered a{color:var(--accent);text-decoration:none;font-weight:var(--w-med)}
    ...
    /* ═══════════════════════════════════════════════════════════════════
       TOAST — a floating shard
       ═══════════════════════════════════════════════════════════════════ */
    .asw-toast{position:absolute;top:var(--sp-4);left:var(--sp-5);right:var(--sp-5);z-index:50;
      display:flex;align-items:center;justify-content:center;gap:var(--sp-2);
      padding:var(--sp-3) var(--sp-4);border-radius:var(--r-md);
      background:var(--pane);color:var(--ink);border:1px solid var(--edge-soft);
      backdrop-filter:var(--blur-4);-webkit-backdrop-filter:var(--blur-4);
      box-shadow:var(--sh-lg),var(--sheen);
      font-size:var(--t-sm);font-weight:var(--w-med);line-height:1.6;text-align:center;
      opacity:0;transform:translateY(-10px) scale(.97);
      transition:opacity var(--ms) var(--ease),transform var(--mm) var(--ease-out);pointer-events:none}
    .asw-toast.show{opacity:1;transform:translateY(0) scale(1)}
    .asw-toast svg{width:14px;height:14px;flex:none;color:var(--accent)}

    /* ═══════════════════════════════════════════════════════════════════
       CITATIONS
       ═══════════════════════════════════════════════════════════════════ */
    .asw-citations{display:flex;flex-wrap:wrap;align-items:center;gap:var(--sp-2);margin-top:var(--sp-3);
      padding-top:var(--sp-3);border-top:1px solid var(--edge-dim)}
    .asw-citations-label{font-size:10px;font-weight:var(--w-bold);color:var(--ink-3)}
    .asw-citation{display:inline-flex;align-items:center;gap:var(--sp-1);height:26px;font-size:var(--t-xs);
      ...
      transition:background var(--ms) var(--ease)}
    .asw-citation:hover{background:var(--g-hover);color:var(--ink)}
    ...
    .asw-handoff{flex:1 1 100%;display:flex;flex-wrap:wrap;align-items:center;gap:var(--sp-2);margin-top:var(--sp-1);
      padding:var(--sp-3);border:1px solid var(--edge-soft);border-radius:var(--r-md);
      background:var(--g-2);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      box-shadow:var(--sheen-sm);
      animation:gl-fade var(--mm) var(--ease-out) both}
    .asw-handoff-label{width:100%;font-size:var(--t-sm);line-height:1.75;color:var(--ink-2);font-weight:var(--w-reg)}
    .asw-handoff-btn{display:inline-flex;align-items:center;gap:var(--sp-1);height:30px;padding:0 var(--sp-3);
      border:1px solid var(--edge-soft);border-radius:var(--r-pill);
      background:var(--g-3);color:var(--ink);
      font-family:inherit;font-size:var(--t-xs);font-weight:var(--w-med);cursor:pointer;text-decoration:none;
      transition:background var(--ms) var(--ease),transform var(--ms) var(--ease-out)}
    .asw-handoff-btn:hover{background:var(--g-hover);transform:translateY(-1px)}
    .asw-handoff-btn:active{transform:translateY(0)}
    ...
    .asw-rule-hint{margin-top:var(--sp-2);padding:var(--sp-2) var(--sp-3);
      background:var(--g-2);border:1px solid var(--edge-dim);border-radius:var(--r-sm);
      font-size:var(--t-sm);line-height:1.75;color:var(--ink-2)}
    .asw-messages > .asw-rule-hint{align-self:center;margin:0;padding:3px var(--sp-3);
      border-radius:var(--r-pill);font-size:10px;color:var(--ink-3);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      animation:gl-fade var(--mm) both}

    /* ═══════════════════════════════════════════════════════════════════
       SHEET — heaviest frost
       ═══════════════════════════════════════════════════════════════════ */
    ...
    .asw-sheet::before{content:"";position:absolute;inset:0;background:rgba(12,14,30,.3);
    ...
    @supports not ((backdrop-filter:blur(1px)) or (-webkit-backdrop-filter:blur(1px))){
      .asw-sheet-card{background:var(--pane-solid)}
    }
    .asw-sheet.open .asw-sheet-card{transform:translateY(0)}
    .asw-sheet-grip{width:38px;height:4px;border-radius:var(--r-pill);background:var(--edge);
      margin:0 auto var(--sp-4)}
    .asw-sheet-close{position:absolute;top:var(--sp-3);inset-inline-end:var(--sp-4);width:32px;height:32px;
    ...
    .asw-sheet-close:hover{background:var(--g-hover);border-color:var(--edge-dim);color:var(--ink)}
    ...
    .asw-sheet-desc{font-size:var(--t-sm);color:var(--ink-2);line-height:1.8;margin:var(--sp-1) 0 var(--sp-4)}
    ...
    .asw-chan{position:relative;display:flex;align-items:center;gap:var(--sp-3);padding:var(--sp-3);
      ...
      background:var(--g-2);text-decoration:none;
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      box-shadow:var(--sheen-sm);
      transition:background var(--ms) var(--ease),transform var(--ms) var(--ease-out),box-shadow var(--ms) var(--ease)}
    .asw-chan:hover{background:var(--g-hover);transform:translateY(-2px);box-shadow:var(--sh-md),var(--sheen-sm)}
    .asw-chan:active{transform:translateY(0)}
    .asw-chan-ic{width:34px;height:34px;flex:none;border-radius:var(--r-sm);display:grid;place-items:center;
      color:#fff;background:var(--chan,var(--accent));
      border:1px solid rgba(255,255,255,.3);box-shadow:var(--sheen-sm)}
    ...
    .asw-flabel{display:flex;align-items:center;gap:var(--sp-1);font-size:var(--t-xs);font-weight:var(--w-bold);
      color:var(--ink-2);margin-bottom:var(--sp-1);padding-inline-start:var(--sp-1)}
    ...
    .asw-form input,.asw-form textarea{width:100%;font-family:inherit;font-size:var(--t-md);color:var(--ink);
      ...
      transition:background var(--ms) var(--ease),border-color var(--ms) var(--ease),box-shadow var(--ms) var(--ease)}
    ...
    .asw-form input:focus,.asw-form textarea:focus{background:var(--g-3);
      border-color:color-mix(in srgb,var(--accent) 45%,transparent);
      box-shadow:0 0 0 3px color-mix(in srgb,var(--accent) 15%,transparent)}
    .asw-field.invalid input,.asw-field.invalid textarea{border-color:color-mix(in srgb,var(--err) 55%,transparent);
      box-shadow:0 0 0 3px color-mix(in srgb,var(--err) 14%,transparent)}
    .asw-field.ok input{border-color:color-mix(in srgb,var(--ok) 50%,transparent)}
    .asw-ferr{display:none;font-size:10.5px;font-weight:var(--w-bold);color:var(--err);margin-top:var(--sp-1);
    ...
    .asw-form-submit{position:relative;width:100%;display:inline-flex;align-items:center;justify-content:center;
      gap:var(--sp-2);margin-top:var(--sp-2);height:46px;padding:0 var(--sp-6);overflow:hidden;
      border:1px solid rgba(255,255,255,.4);border-radius:var(--r-pill);
      background:linear-gradient(135deg,var(--accent),var(--accent-2));color:#fff;
      box-shadow:var(--sh-md),inset 0 1px 0 rgba(255,255,255,.45);
      font-family:inherit;font-size:var(--t-md);font-weight:var(--w-bold);cursor:pointer;
      transition:transform var(--ms) var(--ease-out),box-shadow var(--ms) var(--ease)}
    .asw-form-submit:hover{transform:translateY(-1px);box-shadow:var(--sh-lg),inset 0 1px 0 rgba(255,255,255,.45)}
    .asw-form-submit:active{transform:translateY(0) scale(.99)}
    .asw-form-submit:disabled{background:var(--g-2);color:var(--ink-3);border-color:var(--edge-dim);
      cursor:default;box-shadow:none;transform:none}
    .asw-form-submit svg{width:15px;height:15px;position:relative;z-index:1}
    .asw-form-submit span{position:relative;z-index:1}
    .asw-spinner{width:15px;height:15px;flex:none;border:2px solid color-mix(in srgb,currentColor 28%,transparent);
      border-top-color:currentColor;border-radius:var(--r-pill);animation:gl-spin .8s linear infinite;position:relative;z-index:1}
    .asw-form-note{display:flex;align-items:center;justify-content:center;gap:var(--sp-1);font-size:10px;
      color:var(--ink-3);text-align:center;margin-top:var(--sp-3);line-height:1.7}
    ...
    .asw-form-success{display:flex;flex-direction:column;align-items:center;text-align:center;gap:var(--sp-2);
      padding:var(--sp-6) var(--sp-2) var(--sp-3);animation:gl-fade var(--mm) var(--ease-out) both}
    .asw-checkmark{width:58px;height:58px;stroke:var(--ok);stroke-width:3;fill:none;
      stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 2px 8px color-mix(in srgb,var(--ok) 40%,transparent))}
    .asw-checkmark circle{stroke-dasharray:170;stroke-dashoffset:170;animation:gl-draw 700ms var(--ease-out) 60ms forwards}
    .asw-checkmark path{stroke-dasharray:44;stroke-dashoffset:44;animation:gl-draw 400ms var(--ease-out) 600ms forwards}
    ...
    .asw-fs-close{margin-top:var(--sp-3);height:36px;padding:0 var(--sp-6);
      border:1px solid var(--edge-soft);border-radius:var(--r-pill);
      background:var(--g-3);color:var(--ink);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      font-family:inherit;font-size:var(--t-sm);font-weight:var(--w-bold);cursor:pointer;
      transition:background var(--ms) var(--ease)}
    .asw-fs-close:hover{background:var(--g-hover)}
    .asw-leadform-wrap{animation:gl-fade var(--mm) var(--ease-out) both}
    .asw-leadform{position:relative;flex:1 1 100%;margin:var(--sp-1) 0;padding:var(--sp-4);
      border:1px solid var(--edge-soft);border-radius:var(--r-md);
      background:var(--g-2);
      backdrop-filter:var(--blur-3);-webkit-backdrop-filter:var(--blur-3);
      box-shadow:var(--sheen-sm);animation:gl-fade var(--mm) var(--ease-out) both}
    ...
    @keyframes gl-in{from{opacity:0;transform:scale(.7)}to{opacity:1;transform:scale(1)}}
    @keyframes gl-pop{from{transform:scale(.4)}to{transform:scale(1)}}
    @keyframes gl-spin{to{transform:rotate(360deg)}}
    @keyframes gl-ping{0%{transform:scale(1);opacity:.7}70%,100%{transform:scale(1.4);opacity:0}}
    @keyframes gl-bounce{0%,100%{transform:translateY(0);opacity:.4}50%{transform:translateY(-4px);opacity:1}}
    @keyframes gl-blink{50%{opacity:0}}
    @keyframes gl-draw{to{stroke-dashoffset:0}}
    @keyframes gl-wave{0%,100%{transform:scaleY(.3)}50%{transform:scaleY(1)}}
    @keyframes gl-float{0%,100%{transform:translateY(0)}50%{transform:translateY(-7px)}}
    @keyframes gl-drift-a{0%{transform:translate(0,0) scale(1)}100%{transform:translate(-36px,44px) scale(1.15)}}
    @keyframes gl-drift-b{0%{transform:translate(0,0) scale(1)}100%{transform:translate(40px,-38px) scale(1.2)}}
    ...
    @media(max-width:480px){
      ...
      .asw-header,.asw-footer,.asw-messages,.asw-suggestions{padding-inline:var(--sp-4)}
      .asw-row{max-width:92%}
    }
    @media(prefers-reduced-motion:reduce){
      .asw-panel,.asw-fab,.asw-suggestion,.asw-teaser,.asw-scrollbtn,.asw-toast,.asw-glow,.asw-fab-label,
      .asw-sheet,.asw-sheet-card,.asw-chan,.asw-send,.asw-input-wrap{transition:none}
      .asw-typing-dot,.asw-fab-pulse,.asw-wave span,.asw-spinner,.asw-hero-p,
      .asw-panel::before,.asw-panel::after{animation:none}
    ...
    .asw :focus-visible{outline:2px solid color-mix(in srgb,var(--accent) 80%,transparent);outline-offset:2px}
