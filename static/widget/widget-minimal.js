...
(function () {
  ...
  var CSS = `
    /* ═══════════════════════════════════════════════════════════════════════
       AI Support Widget — MINIMALISM / CLEAN UI
       ───────────────────────────────────────────────────────────────────────
       Principle: "Less, but better."

         · Monochrome    — a single grey ramp. Brand colour appears in at most
                           two places at a time, never as a fill on a surface.
         · White space   — generous, deliberate. Padding roughly doubled;
                           the empty area is the primary design element.
         · Hairlines     — 1px rules replace every card, shadow and gradient.
                           Exactly one soft shadow exists (the floating panel).
         · Typography    — one family, three weights, tight tracking, a strict
                           four-step size ramp. Hierarchy comes from weight and
                           space, never from colour or decoration.
         · Restraint     — no rounded blobs, no glows, no beams, no aurora.
                           Motion is opacity and small translation only.
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    /* ───────────────────────────────────────────────────────────────────
       SCALE — one grey ramp, one accent, four sizes, three weights
       ─────────────────────────────────────────────────────────────────── */
    .asw{
      --asw-primary:#111111;--asw-secondary:#111111;--asw-accent:#111111;
      ...
      --g-3:#e8e8e8;   /* hairline */
      --g-4:#d4d4d4;
      --g-5:#8a8a8a;   /* secondary ink */
      --g-6:#5c5c5c;
      --g-7:#111111;   /* primary ink */
      ...
      /* status — desaturated, used only as small marks */
      --asw-success:#2f7a52;--asw-error:#b23b3b;--asw-warning:#8a6d2f;--online:#3f9e6a;

      /* space — deliberately large */
      --sp-1:4px;--sp-2:8px;--sp-3:12px;--sp-4:16px;--sp-5:20px;
      --sp-6:24px;--sp-7:28px;--sp-8:32px;--sp-10:40px;--sp-12:48px;--sp-16:64px;

      /* type — four steps only */
      --t-xs:11px;--t-sm:12.5px;--t-md:14px;--t-lg:16px;--t-xl:19px;
      --w-normal:400;--w-medium:500;--w-bold:600;

      /* geometry — near-square */
      --rad:2px;--rad-2:4px;--rad-pill:999px;

      /* the single shadow in the entire widget */
      --lift:0 1px 2px rgba(0,0,0,.04),0 12px 32px -12px rgba(0,0,0,.16);

      /* motion — opacity and small translation only */
      --ms:140ms;--mm:220ms;--ml:360ms;
      --ease:cubic-bezier(.4,0,.2,1);
      --ease-out:cubic-bezier(.16,1,.3,1);

      /* aliases consumed by the unchanged JS */
      --asw-bg:var(--paper);--asw-surface:var(--paper-2);--asw-surface-2:var(--g-2);
      --asw-text:var(--ink);--asw-text-secondary:var(--ink-2);--asw-text-muted:var(--ink-3);
      --asw-border:var(--rule);--asw-border-strong:var(--rule-2);
      --asw-bot-bg:var(--paper);--asw-bot-text:var(--ink);
      --asw-user-bg:var(--ink);--asw-user-bg-2:var(--ink);--asw-user-text:var(--paper);
      --asw-input-bg:var(--paper);--asw-input-focus-bg:var(--paper);
      --asw-shadow:var(--lift);--asw-header-bg:var(--paper);
      --asw-radius:var(--rad);--asw-radius-sm:var(--rad);--asw-radius-xs:var(--rad);
    ...
    /* dark — the same ramp, inverted. Still monochrome. */
    .asw.dark{
      --g-0:#0a0a0a;
      --g-1:#101010;
      --g-2:#161616;
      ...
      --g-6:#b8b8b8;
      --g-7:#f2f2f2;
      --asw-success:#5cb98a;--asw-error:#e08585;--asw-warning:#c9a86a;--online:#5cb98a;
      --lift:0 1px 2px rgba(0,0,0,.5),0 12px 32px -12px rgba(0,0,0,.7);
    }
    ...
    /* removed entirely — minimalism has no ambient glow */
    .asw-glow{display:none}

    /* ───────────────────────────────────────────────────────────────────
       LAUNCHER — a plain circle. One shape, one colour.
       ─────────────────────────────────────────────────────────────────── */
    .asw-fab{position:fixed;width:52px;height:52px;border:1px solid var(--ink);border-radius:var(--rad-pill);background:var(--ink);color:var(--paper);display:grid;place-items:center;cursor:pointer;z-index:2;box-shadow:none;transition:background var(--mm) var(--ease),color var(--mm) var(--ease),border-color var(--mm) var(--ease),opacity var(--ms) var(--ease);animation:m-fade var(--ml) var(--ease-out) 100ms both}
    .asw-fab:hover{background:var(--paper);color:var(--ink)}
    .asw-fab:active{opacity:.7}
    .asw-fab.active{background:var(--paper);color:var(--ink);border-color:var(--rule-2)}
    .asw-fab svg{width:21px;height:21px}
    .asw-fab-shine,.asw-fab-pulse{display:none}
    .asw-fab-icon-main,.asw-fab-icon-close{position:absolute;inset:0;display:grid;place-items:center;transition:opacity var(--ms) var(--ease)}
    .asw-fab-icon-close{opacity:0}
    .asw-fab.active .asw-fab-icon-main{opacity:0}
    .asw-fab.active .asw-fab-icon-close{opacity:1}
    .asw-fab-custom-icon{width:22px;height:22px;object-fit:contain}

    /* badge — a small dot with a number, no fill colour */
    .asw-fab-badge{position:absolute;top:-2px;inset-inline-end:-2px;min-width:17px;height:17px;padding:0 4px;border-radius:var(--rad-pill);background:var(--paper);color:var(--ink);font-size:10px;font-weight:var(--w-bold);line-height:1;display:grid;place-items:center;border:1px solid var(--ink);pointer-events:none}
    .asw-fab-badge[hidden]{display:none}
    .asw-fab-badge.asw-pop{animation:m-fade var(--mm) var(--ease-out)}

    /* label — plain text, no bubble */
    .asw-fab-label{position:absolute;inset-inline-end:calc(100% + var(--sp-3));top:50%;transform:translateY(-50%);white-space:nowrap;padding:0;background:none;color:var(--ink);font-family:inherit;font-size:var(--t-sm);font-weight:var(--w-medium);box-shadow:none;opacity:0;pointer-events:none;transition:opacity var(--ms) var(--ease)}
    .corner-bl .asw-fab-label,.corner-tl .asw-fab-label{inset-inline-end:auto;inset-inline-start:calc(100% + var(--sp-3))}
    .asw-fab:hover .asw-fab-label,.asw-fab:focus-visible .asw-fab-label{opacity:1}
    .asw-fab.active .asw-fab-label{display:none}

    /* ───────────────────────────────────────────────────────────────────
       TEASER — hairline box, no shadow
       ─────────────────────────────────────────────────────────────────── */
    .asw-teaser{position:fixed;z-index:1;max-width:260px;display:flex;align-items:flex-start;gap:var(--sp-3);padding:var(--sp-4);border-radius:var(--rad);background:var(--paper);border:1px solid var(--rule);box-shadow:none;cursor:pointer;opacity:0;transform:translateY(4px);transition:opacity var(--mm) var(--ease),transform var(--mm) var(--ease-out),border-color var(--ms) var(--ease);pointer-events:none}
    .asw-teaser:hover{border-color:var(--rule-2)}
    .asw-teaser.show{opacity:1;transform:translateY(0);pointer-events:auto}
    .asw-teaser-orb{width:28px;height:28px;flex:none;border-radius:var(--rad-pill);background:none;border:1px solid var(--rule-2);color:var(--ink);display:grid;place-items:center}
    .asw-teaser-orb svg{width:13px;height:13px}
    .asw-teaser-body{min-width:0;padding-inline-end:var(--sp-4)}
    .asw-teaser-title{font-size:var(--t-md);font-weight:var(--w-medium);color:var(--ink);letter-spacing:-.015em}
    .asw-teaser-text{font-size:var(--t-sm);color:var(--ink-2);line-height:1.7;margin-top:var(--sp-1)}
    .asw-teaser-close{position:absolute;top:var(--sp-2);inset-inline-start:var(--sp-2);width:20px;height:20px;border:none;border-radius:var(--rad);background:none;color:var(--ink-3);display:grid;place-items:center;cursor:pointer;opacity:0;transition:opacity var(--ms) var(--ease),color var(--ms) var(--ease)}
    .asw-teaser:hover .asw-teaser-close,.asw-teaser-close:focus-visible{opacity:1}
    .asw-teaser-close:hover{color:var(--ink)}
    .asw-teaser-close svg{width:10px;height:10px}
    @media(max-width:520px){.asw-teaser{display:none}}

    /* ───────────────────────────────────────────────────────────────────
       PANEL — the one element permitted a shadow
       ─────────────────────────────────────────────────────────────────── */
    .asw-panel{position:fixed;z-index:3;width:var(--asw-panel-w);height:var(--asw-panel-h);max-width:calc(100vw - var(--sp-5));max-height:calc(100dvh - 96px);display:flex;flex-direction:column;overflow:hidden;background:var(--paper);border:1px solid var(--rule);border-radius:var(--rad);box-shadow:var(--lift);opacity:0;transform:translateY(8px);pointer-events:none;visibility:hidden;transition:opacity var(--mm) var(--ease),transform var(--mm) var(--ease),visibility 0s linear var(--mm)}
    .asw-panel.open{opacity:1;transform:translateY(0);pointer-events:auto;visibility:visible;transition:opacity var(--mm) var(--ease),transform var(--ml) var(--ease-out),visibility 0s}
    .asw-panel.no-anim{transition:none}
    .asw-root.corner-tr .asw-panel,.asw-root.corner-tl .asw-panel{transform:translateY(-8px)}
    .asw-root.corner-tr .asw-panel.open,.asw-root.corner-tl .asw-panel.open{transform:translateY(0)}

    /* ───────────────────────────────────────────────────────────────────
       HEADER — a line of text and a rule. Nothing more.
       ─────────────────────────────────────────────────────────────────── */
    .asw-header{position:relative;z-index:4;flex:none;display:flex;align-items:center;gap:var(--sp-3);padding:var(--sp-5) var(--sp-6);background:var(--paper);border-bottom:1px solid var(--rule)}
    ...
    .asw-avatar{width:30px;height:30px;border-radius:var(--rad-pill);display:grid;place-items:center;background:none;border:1px solid var(--rule-2);color:var(--ink);font-size:var(--t-sm);font-weight:var(--w-medium);overflow:hidden}
    .asw-avatar img{width:100%;height:100%;border-radius:inherit;object-fit:cover}
    .asw-status{position:absolute;bottom:-1px;inset-inline-end:-1px;width:8px;height:8px;border-radius:var(--rad-pill);background:var(--online);border:1.5px solid var(--paper)}
    .asw-info{min-width:0;flex:1}
    .asw-title-row{display:flex;align-items:baseline;gap:var(--sp-2);min-width:0}
    .asw-title{font-size:var(--t-md);font-weight:var(--w-medium);letter-spacing:-.018em;line-height:1.4;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--ink)}
    .asw-subtitle{font-size:var(--t-xs);font-weight:var(--w-normal);color:var(--ink-2);margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;letter-spacing:0}

    /* the AI chip is redundant next to the title — hidden */
    .asw-ai-chip{display:none}
    /* the status badge reduces to a bare word */
    .asw-badge{display:inline-flex;align-items:center;gap:var(--sp-1);flex:none;padding:0;border:none;background:none;font-size:var(--t-xs);font-weight:var(--w-normal);color:var(--ink-2);white-space:nowrap}
    .asw-badge-dot{width:4px;height:4px;border-radius:var(--rad-pill);background:var(--online);flex:none}

    /* icon buttons — no background until hovered */
    .asw-actions{position:relative;display:flex;gap:var(--sp-1);flex:none}
    .asw-header-btn{width:30px;height:30px;border:none;border-radius:var(--rad);background:none;color:var(--ink-2);display:grid;place-items:center;cursor:pointer;transition:color var(--ms) var(--ease),background var(--ms) var(--ease)}
    .asw-header-btn:hover{color:var(--ink);background:var(--g-2)}
    .asw-header-btn:active{opacity:.6}
    .asw-header-btn svg{width:15px;height:15px}

    /* ───────────────────────────────────────────────────────────────────
       OFFLINE BANNER — a thin line of text
       ─────────────────────────────────────────────────────────────────── */
    .asw-netstatus{position:relative;z-index:3;flex:none;display:flex;align-items:center;justify-content:center;gap:var(--sp-2);padding:var(--sp-2) var(--sp-4);background:none;border-bottom:1px solid var(--rule);color:var(--ink-2);font-size:var(--t-xs);font-weight:var(--w-normal)}
    .asw-netstatus[hidden]{display:none}
    .asw-netstatus svg{width:12px;height:12px;flex:none}

    /* ───────────────────────────────────────────────────────────────────
       MESSAGES — the content. Everything above serves this.
       ─────────────────────────────────────────────────────────────────── */
    .asw-messages{position:relative;z-index:1;flex:1;min-height:0;overflow-y:auto;overflow-x:hidden;overscroll-behavior:contain;padding:var(--sp-7) var(--sp-6) var(--sp-4);display:flex;flex-direction:column;gap:var(--sp-6);scroll-behavior:smooth;scrollbar-width:thin;scrollbar-color:var(--rule-2) transparent;background:var(--paper)}
    .asw-messages::-webkit-scrollbar{width:4px}
    .asw-messages::-webkit-scrollbar-track{background:transparent}
    .asw-messages::-webkit-scrollbar-thumb{background:var(--rule-2);border-radius:var(--rad-pill)}

    .asw-row{position:relative;z-index:1;display:flex;flex-wrap:wrap;align-items:flex-start;gap:var(--sp-3);max-width:90%;animation:m-rise var(--ml) var(--ease-out) both}
    .asw-row.user{align-self:flex-end;flex-direction:row-reverse}
    .asw-row.bot{align-self:flex-start}
    .asw-row.bot + .asw-row.bot,.asw-row.user + .asw-row.user{margin-top:calc(var(--sp-4) * -1)}
    .asw-row.asw-cont .asw-msg-avatar{visibility:hidden}
    .asw-row.no-anim{animation:none}

    /* avatar — a small outlined circle, no fill */
    .asw-msg-avatar{width:24px;height:24px;flex:0 0 24px;margin-top:var(--sp-1);border-radius:var(--rad-pill);display:grid;place-items:center;font-size:10px;font-weight:var(--w-medium);overflow:hidden;background:none;border:1px solid var(--rule-2);color:var(--ink-2)}
    .asw-row.bot .asw-msg-avatar img{width:100%;height:100%;object-fit:cover}
    .asw-row.user .asw-msg-avatar{border-color:var(--ink);color:var(--ink)}
    .asw-no-avatar .asw-msg-avatar{display:none}

    /* divider — a centred word between two hairlines */
    .asw-divider{position:relative;z-index:1;align-self:stretch;display:flex;align-items:center;gap:var(--sp-4);margin:var(--sp-2) 0;color:var(--ink-2);font-size:var(--t-xs);font-weight:var(--w-normal);letter-spacing:.02em;animation:m-fade var(--mm) var(--ease-out) both}
    .asw-divider::before,.asw-divider::after{content:"";flex:1;height:1px;background:var(--rule)}

    /* ── BUBBLES — the bot has none at all ──
       A reply is simply text on the page. Only the user's own message is
       marked, with a solid ink block, so the two voices stay legible. */
    ...
    .asw-bubble{padding:0;font-size:var(--asw-font-size,14px);line-height:1.9;word-break:break-word;white-space:pre-wrap;overflow-wrap:anywhere;position:relative;text-align:start}
    .asw-row.bot .asw-bubble{background:none;border:none;border-radius:0;color:var(--ink);padding:0;box-shadow:none}
    .asw-row.user .asw-bubble{background:var(--ink);color:var(--paper);border:none;border-radius:var(--rad-2);padding:var(--sp-3) var(--sp-4);box-shadow:none}
    .asw[data-bubble="sharp"] .asw-row.user .asw-bubble{border-radius:0}
    .asw[data-bubble="pill"] .asw-row.user .asw-bubble{border-radius:var(--rad-pill);padding:var(--sp-3) var(--sp-5)}
    .asw-bubble.asw-error-bubble{background:none;border:none;border-inline-start:2px solid var(--asw-error);border-radius:0;padding-inline-start:var(--sp-4);color:var(--asw-error)}

    /* rich text */
    .asw-bubble strong{font-weight:var(--w-bold)}
    .asw-bubble em{font-style:italic}
    .asw-bubble a{color:inherit;font-weight:var(--w-normal);text-decoration:underline;text-underline-offset:3px;text-decoration-color:var(--rule-2);transition:text-decoration-color var(--ms) var(--ease)}
    .asw-bubble a:hover{text-decoration-color:currentColor}
    .asw-bubble ul,.asw-bubble ol{margin:var(--sp-3) 0;padding-inline-start:var(--sp-5)}
    .asw-bubble li{margin:var(--sp-2) 0;padding-inline-start:var(--sp-1)}
    .asw-bubble li::marker{color:var(--ink-3)}
    .asw-bubble p{margin:var(--sp-3) 0}
    .asw-bubble p:first-child{margin-top:0}
    .asw-bubble p:last-child{margin-bottom:0}
    .asw-bubble code:not(.asw-code-block code){padding:1px 5px;border-radius:var(--rad);background:var(--g-2);border:none;font-family:ui-monospace,'SF Mono',Menlo,Consolas,monospace;font-size:.88em;color:inherit;white-space:pre-wrap}
    .asw-row.user .asw-bubble code{background:rgba(255,255,255,.16)}

    /* tables — rules only, no fills */
    .asw-table-wrap{margin:var(--sp-4) 0;border:none;border-top:1px solid var(--rule);border-bottom:1px solid var(--rule);border-radius:0;overflow:hidden;overflow-x:auto;background:none;direction:rtl}
    .asw-table-wrap table{border-collapse:collapse;width:100%;font-size:var(--t-sm);line-height:1.8}
    .asw-table-wrap th{background:none;font-weight:var(--w-medium);text-align:start;padding:var(--sp-3) var(--sp-3);white-space:nowrap;color:var(--ink);border-bottom:1px solid var(--rule)}
    .asw-table-wrap td{padding:var(--sp-3) var(--sp-3);border-top:1px solid var(--rule);color:var(--ink-2)}
    .asw-table-wrap tbody tr:first-child td{border-top:none}
    .asw-table-wrap th[style*="center"],.asw-table-wrap td[style*="center"]{text-align:center}

    /* code — a bordered block on the page background */
    .asw-code-wrap{position:relative;margin:var(--sp-4) 0;border-radius:var(--rad);overflow:hidden;background:var(--g-1);border:1px solid var(--rule);direction:ltr;text-align:left}
    .asw-code-header{display:flex;align-items:center;gap:var(--sp-2);padding:var(--sp-2) var(--sp-3);background:none;border-bottom:1px solid var(--rule)}
    .asw-code-dots{display:none}
    .asw-code-lang{font-size:10px;color:var(--ink-2);font-weight:var(--w-normal);letter-spacing:.1em;text-transform:uppercase}
    .asw-code-copy{margin-left:auto;display:inline-flex;align-items:center;gap:var(--sp-1);height:22px;padding:0 var(--sp-2);border:none;border-radius:var(--rad);background:none;color:var(--ink-2);font-size:10.5px;font-weight:var(--w-normal);cursor:pointer;font-family:inherit;transition:color var(--ms) var(--ease)}
    .asw-code-copy:hover{color:var(--ink)}
    .asw-code-copy.copied{color:var(--asw-success)}
    .asw-code-copy svg{width:11px;height:11px}
    .asw-code-block{margin:0;padding:var(--sp-4);overflow-x:auto;font-family:ui-monospace,'SF Mono',Menlo,Consolas,monospace;font-size:var(--t-sm);line-height:1.8;color:var(--ink);white-space:pre;tab-size:2;background:none}
    .asw-code-block::-webkit-scrollbar{height:4px}
    .asw-code-block::-webkit-scrollbar-thumb{background:var(--rule-2);border-radius:var(--rad-pill)}
    .asw-code-block code{font-family:inherit}

    /* timestamp — very quiet */
    .asw-time{padding:var(--sp-2) 0 0;font-size:10.5px;color:var(--ink-3);font-weight:var(--w-normal);letter-spacing:.02em;font-variant-numeric:tabular-nums}
    .asw-row.user .asw-time{text-align:left}

    /* feedback — icons only, revealed on hover */
    .asw-feedback{display:flex;align-items:center;gap:var(--sp-1);margin-top:var(--sp-2);opacity:0;transition:opacity var(--ms) var(--ease)}
    ...
    .asw-fb-btn{width:24px;height:24px;border:none;border-radius:var(--rad);background:none;color:var(--ink-3);cursor:pointer;display:grid;place-items:center;transition:color var(--ms) var(--ease)}
    .asw-fb-btn:hover{color:var(--ink)}
    .asw-fb-btn:active{opacity:.6}
    .asw-fb-btn.active{color:var(--ink)}
    .asw-fb-btn.active.thumbs-up,.asw-fb-btn.copied{color:var(--asw-success)}
    .asw-fb-btn.active.thumbs-down{color:var(--asw-error)}
    .asw-fb-btn svg{width:13px;height:13px}

    /* retry — a text link, not a button */
    .asw-retry{display:inline-flex;align-items:center;gap:var(--sp-2);margin-top:var(--sp-3);padding:0;border:none;background:none;color:var(--ink);font-family:inherit;font-size:var(--t-sm);font-weight:var(--w-medium);cursor:pointer;text-decoration:underline;text-underline-offset:3px;text-decoration-color:var(--rule-2);transition:text-decoration-color var(--ms) var(--ease)}
    .asw-retry:hover{text-decoration-color:currentColor}
    .asw-retry svg{width:12px;height:12px}

    /* ───────────────────────────────────────────────────────────────────
       TYPING — three small dots, nothing else
       ─────────────────────────────────────────────────────────────────── */
    .asw-typing{position:relative;z-index:1;flex:none;display:flex;align-items:center;gap:var(--sp-3);padding:0 var(--sp-6) var(--sp-4)}
    .asw-typing[hidden]{display:none}
    .asw-typing-ava{display:none}
    .asw-typing-bubble{display:flex;align-items:center;gap:var(--sp-1);padding:0;background:none;border:none;border-radius:0;box-shadow:none}
    .asw-typing-dot{width:4px;height:4px;border-radius:var(--rad-pill);background:var(--ink-3);animation:m-bounce 1.2s var(--ease) infinite}
    .asw-typing-dot:nth-child(2){animation-delay:.15s}
    .asw-typing-dot:nth-child(3){animation-delay:.3s}
    .asw-typing-label{font-size:var(--t-xs);font-weight:var(--w-normal);color:var(--ink-2);margin-inline-start:var(--sp-2)}

    /* streaming caret — a thin bar */
    .asw-bubble.streaming::after{content:"";display:inline-block;width:1.5px;height:14px;margin-inline-start:2px;background:var(--ink);vertical-align:-2px;animation:m-blink 1s steps(2) infinite}

    /* ───────────────────────────────────────────────────────────────────
       HERO — maximum white space. A greeting and four quiet lines.
       ─────────────────────────────────────────────────────────────────── */
    .asw-hero{position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;text-align:center;gap:var(--sp-6);padding:var(--sp-16) var(--sp-4) var(--sp-6);transition:opacity var(--mm) var(--ease),transform var(--mm) var(--ease)}
    .asw-hero>*{animation:m-rise var(--ml) var(--ease-out) both}
    ...
    .asw-hero-out{opacity:0!important;transform:translateY(-6px)!important;pointer-events:none}
    .asw-hero-spot,.asw-hero-p{display:none}
    .asw-hero-orb-wrap{position:relative}
    .asw-hero-orb{position:relative;width:44px;height:44px;border-radius:var(--rad-pill);background:none;border:1px solid var(--rule-2);color:var(--ink);display:grid;place-items:center;box-shadow:none}
    .asw-hero-orb svg{width:19px;height:19px}
    .asw-hero-orb img{width:100%;height:100%;border-radius:inherit;object-fit:cover}
    .asw-hero-text{font-size:var(--t-lg);font-weight:var(--w-normal);color:var(--ink);line-height:1.85;max-width:280px;letter-spacing:-.018em}

    /* hero prompts — a stacked list separated by hairlines, not cards */
    .asw-hero-grid{display:flex;flex-direction:column;gap:0;width:100%;max-width:320px;margin-top:var(--sp-2);border-top:1px solid var(--rule)}
    .asw-hero-grid .asw-suggestion{height:auto;justify-content:space-between;text-align:start;white-space:normal;border:none;border-bottom:1px solid var(--rule);border-radius:0;padding:var(--sp-4) var(--sp-1);line-height:1.7;font-size:var(--t-md);font-weight:var(--w-normal);gap:var(--sp-3);background:none;color:var(--ink)}
    .asw-hero-grid .asw-suggestion::before{display:none}
    .asw-hero-grid .asw-suggestion:hover{background:none;color:var(--ink);transform:none;box-shadow:none}
    .asw-hero-grid .asw-suggestion:hover .asw-card-arrow{opacity:1;transform:translateX(0)}
    .asw-hero-grid .asw-suggestion span{display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
    .asw-card-arrow{margin-inline-start:auto;flex:none;color:var(--ink-3);opacity:0;transform:translateX(3px);transition:opacity var(--ms) var(--ease),transform var(--mm) var(--ease-out)}
    .asw-card-arrow svg{width:13px;height:13px;display:block}

    /* ───────────────────────────────────────────────────────────────────
       SUGGESTIONS — text chips outlined in hairline
       ─────────────────────────────────────────────────────────────────── */
    .asw-suggestions{position:relative;z-index:2;flex:none;display:flex;gap:var(--sp-2);align-items:center;overflow-x:auto;padding:var(--sp-3) var(--sp-6);background:var(--paper);scrollbar-width:none;-webkit-overflow-scrolling:touch}
    .asw-suggestions::-webkit-scrollbar{display:none}
    .asw-suggestions[hidden]{display:none}
    .asw-suggestion{display:inline-flex;align-items:center;gap:var(--sp-2);flex:0 0 auto;height:28px;padding:0 var(--sp-3);border-radius:var(--rad-pill);border:1px solid var(--rule);background:none;color:var(--ink-2);font-family:inherit;font-size:var(--t-sm);font-weight:var(--w-normal);cursor:pointer;white-space:nowrap;text-decoration:none;transition:border-color var(--ms) var(--ease),color var(--ms) var(--ease)}
    .asw-suggestion:hover{border-color:var(--ink);color:var(--ink)}
    .asw-suggestion:active{opacity:.6}
    .asw-bubble .asw-suggestion{margin-top:var(--sp-2)}

    /* ───────────────────────────────────────────────────────────────────
       COMPOSER — a single rule above, a line of input below
       ─────────────────────────────────────────────────────────────────── */
    .asw-footer{position:relative;z-index:2;flex:none;padding:var(--sp-4) var(--sp-6) calc(var(--sp-4) + env(safe-area-inset-bottom,0px));background:var(--paper);border-top:1px solid var(--rule)}
    .asw-input-wrap{display:flex;align-items:flex-end;gap:var(--sp-2);padding:0;border:none;border-radius:0;background:none;transition:none}
    .asw-input-wrap:focus-within{background:none;border:none;box-shadow:none}
    .asw-input-wrap.asw-listening .asw-mic{color:var(--asw-error)}

    .asw-input{min-width:0;flex:1;border:0;outline:0;background:none;color:var(--ink);font-family:inherit;font-size:var(--asw-font-size,14px);line-height:1.7;resize:none;padding:var(--sp-2) 0;max-height:132px;overflow-y:auto;scrollbar-width:none}
    .asw-input::-webkit-scrollbar{display:none}
    .asw-input::placeholder{color:var(--ink-3)}

    .asw-wave{display:flex;align-items:center;gap:2px;height:16px;flex:none;padding-inline-end:var(--sp-1)}
    .asw-wave[hidden]{display:none}
    .asw-wave span{width:2px;height:100%;border-radius:var(--rad-pill);background:var(--ink-3);transform-origin:center;animation:m-wave .9s var(--ease) infinite}
    ...
    .asw-mic{width:30px;height:30px;flex:none;border:none;border-radius:var(--rad);background:none;color:var(--ink-3);display:grid;place-items:center;cursor:pointer;transition:color var(--ms) var(--ease)}
    .asw-mic:hover{color:var(--ink)}
    .asw-mic[hidden]{display:none}
    .asw-mic svg{width:15px;height:15px}

    /* send — a small solid square, the only filled control */
    .asw-send{width:30px;height:30px;flex:none;border:none;border-radius:var(--rad-pill);display:grid;place-items:center;background:var(--ink);color:var(--paper);cursor:pointer;box-shadow:none;transition:opacity var(--ms) var(--ease),background var(--ms) var(--ease)}
    .asw-send:hover{opacity:.8}
    .asw-send:active{opacity:.6}
    .asw-send:disabled{cursor:default;background:none;color:var(--ink-3);opacity:1}
    .asw-send svg{width:15px;height:15px}
    .asw-send.asw-stop{background:var(--asw-error)}

    .asw-scrollbtn{position:absolute;top:-40px;inset-inline-end:var(--sp-6);z-index:5;width:28px;height:28px;border-radius:var(--rad-pill);border:1px solid var(--rule);background:var(--paper);color:var(--ink-2);display:grid;place-items:center;cursor:pointer;box-shadow:none;opacity:0;transform:translateY(4px);pointer-events:none;transition:opacity var(--ms) var(--ease),transform var(--mm) var(--ease-out),color var(--ms) var(--ease)}
    .asw-scrollbtn.show{opacity:1;transform:translateY(0);pointer-events:auto}
    .asw-scrollbtn:hover{color:var(--ink);border-color:var(--rule-2)}
    .asw-scrollbtn svg{width:13px;height:13px}

    .asw-foot-meta{display:flex;flex-direction:column;align-items:center;gap:2px;padding-top:var(--sp-3)}
    .asw-disclaim{font-size:10.5px;color:var(--ink-3);text-align:center;line-height:1.6}
    .asw-powered{text-align:center;font-size:10.5px;color:var(--ink-3);font-weight:var(--w-normal)}
    .asw-powered[hidden]{display:none}
    .asw-powered a{color:var(--ink-2);text-decoration:none;font-weight:var(--w-normal)}
    .asw-powered a:hover{color:var(--ink);text-decoration:underline}
    .asw-resource-links{display:inline-flex;gap:var(--sp-2);margin-inline-start:var(--sp-2)}

    /* ───────────────────────────────────────────────────────────────────
       TOAST — plain inverted bar
       ─────────────────────────────────────────────────────────────────── */
    .asw-toast{position:absolute;top:var(--sp-4);left:var(--sp-6);right:var(--sp-6);z-index:50;display:flex;align-items:center;justify-content:center;gap:var(--sp-2);padding:var(--sp-3) var(--sp-4);border-radius:var(--rad);background:var(--ink);color:var(--paper);font-size:var(--t-sm);font-weight:var(--w-normal);line-height:1.6;text-align:center;border:none;box-shadow:none;opacity:0;transform:translateY(-4px);transition:opacity var(--ms) var(--ease),transform var(--mm) var(--ease-out);pointer-events:none}
    .asw-toast.show{opacity:1;transform:translateY(0)}
    .asw-toast svg{width:13px;height:13px;flex:none;color:currentColor}

    /* ───────────────────────────────────────────────────────────────────
       CITATIONS — a quiet list of underlined references
       ─────────────────────────────────────────────────────────────────── */
    .asw-citations{display:flex;flex-wrap:wrap;align-items:center;gap:var(--sp-3);margin-top:var(--sp-4);padding-top:var(--sp-3);border-top:1px solid var(--rule)}
    .asw-citations-label{font-size:10.5px;font-weight:var(--w-normal);color:var(--ink-3);letter-spacing:.02em}
    .asw-citation{display:inline-flex;align-items:center;gap:var(--sp-1);font-size:var(--t-xs);font-weight:var(--w-normal);color:var(--ink-2);background:none;border:none;padding:0;border-radius:0;text-decoration:underline;text-underline-offset:3px;text-decoration-color:var(--rule-2);max-width:170px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;transition:color var(--ms) var(--ease)}
    .asw-citation:hover{color:var(--ink);text-decoration-color:currentColor}
    .asw-citation svg{width:10px;height:10px;flex:none}

    /* ───────────────────────────────────────────────────────────────────
       HANDOFF — a bordered aside, no fill
       ─────────────────────────────────────────────────────────────────── */
    .asw-handoff{flex:1 1 100%;display:flex;flex-wrap:wrap;align-items:center;gap:var(--sp-3);margin-top:var(--sp-2);padding:var(--sp-4) 0 0;border:none;border-top:1px solid var(--rule);border-radius:0;background:none;animation:m-fade var(--mm) var(--ease-out) both}
    .asw-handoff-label{width:100%;font-size:var(--t-sm);line-height:1.8;color:var(--ink-2);font-weight:var(--w-normal)}
    .asw-handoff-btn{display:inline-flex;align-items:center;gap:var(--sp-2);height:28px;padding:0 var(--sp-3);border-radius:var(--rad-pill);border:1px solid var(--rule);background:none;color:var(--ink);font-family:inherit;font-size:var(--t-xs);font-weight:var(--w-normal);cursor:pointer;text-decoration:none;transition:border-color var(--ms) var(--ease)}
    .asw-handoff-btn:hover{border-color:var(--ink)}
    ...
    .asw-rule-hint{margin-top:var(--sp-3);padding:var(--sp-3) 0 0;background:none;border:none;border-top:1px solid var(--rule);border-radius:0;font-size:var(--t-sm);line-height:1.8;color:var(--ink-2)}
    .asw-messages > .asw-rule-hint{align-self:center;margin:0;padding:0;border:none;color:var(--ink-3);font-size:var(--t-xs);animation:m-fade var(--mm) both}

    /* ───────────────────────────────────────────────────────────────────
       SHEET — a plain panel sliding up
       ─────────────────────────────────────────────────────────────────── */
    .asw-sheet{position:absolute;inset:0;z-index:40;display:flex;align-items:flex-end;opacity:0;pointer-events:none;transition:opacity var(--mm) var(--ease)}
    .asw-sheet::before{content:"";position:absolute;inset:0;background:rgba(0,0,0,.28)}
    .asw.dark .asw-sheet::before{background:rgba(0,0,0,.6)}
    .asw-sheet.open{opacity:1;pointer-events:auto}
    .asw-sheet[hidden]{display:none}
    .asw-sheet-card{position:relative;width:100%;max-height:92%;overflow-y:auto;overscroll-behavior:contain;background:var(--paper);border-radius:var(--rad) var(--rad) 0 0;border-top:1px solid var(--rule);box-shadow:none;padding:var(--sp-5) var(--sp-6) calc(var(--sp-6) + env(safe-area-inset-bottom,0px));transform:translateY(100%);transition:transform var(--ml) var(--ease-out);scrollbar-width:thin}
    .asw-sheet.open .asw-sheet-card{transform:translateY(0)}
    .asw-sheet-grip{width:28px;height:2px;border-radius:var(--rad-pill);background:var(--rule-2);margin:0 auto var(--sp-5)}
    .asw-sheet-close{position:absolute;top:var(--sp-4);inset-inline-end:var(--sp-5);width:26px;height:26px;border:none;border-radius:var(--rad);background:none;color:var(--ink-3);display:grid;place-items:center;cursor:pointer;transition:color var(--ms) var(--ease)}
    .asw-sheet-close:hover{color:var(--ink)}
    .asw-sheet-close svg{width:13px;height:13px}
    .asw-sheet-title{font-size:var(--t-xl);font-weight:var(--w-medium);color:var(--ink);letter-spacing:-.024em;line-height:1.4}
    .asw-sheet-desc{font-size:var(--t-sm);color:var(--ink-2);line-height:1.85;margin:var(--sp-2) 0 var(--sp-6)}

    /* channels — a divided list, not a grid of cards */
    .asw-chans{display:flex;flex-direction:column;gap:0;margin-bottom:var(--sp-6);border-top:1px solid var(--rule)}
    .asw-chan{position:relative;display:flex;align-items:center;gap:var(--sp-3);padding:var(--sp-4) 0;border:none;border-bottom:1px solid var(--rule);border-radius:0;background:none;text-decoration:none;transition:opacity var(--ms) var(--ease)}
    .asw-chan:hover{opacity:.6;transform:none;box-shadow:none}
    .asw-chan-ic{width:28px;height:28px;flex:none;border-radius:var(--rad-pill);display:grid;place-items:center;color:var(--ink-2);background:none;border:1px solid var(--rule-2)}
    .asw-chan-ic svg{width:13px;height:13px}
    .asw-chan-tx{min-width:0;display:flex;flex-direction:column;flex:1}
    .asw-chan-tx b{font-size:var(--t-md);font-weight:var(--w-normal);color:var(--ink);letter-spacing:-.012em}
    .asw-chan-tx i{font-style:normal;font-size:var(--t-xs);color:var(--ink-3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;direction:ltr;text-align:end}
    .asw-chan-more{width:100%;justify-content:center;margin-top:0}

    /* ───────────────────────────────────────────────────────────────────
       FORM — underline fields. No boxes.
       ─────────────────────────────────────────────────────────────────── */
    .asw-form{text-align:start}
    .asw-field{margin-bottom:var(--sp-5)}
    .asw-field-row{display:grid;grid-template-columns:1fr 1fr;gap:var(--sp-4)}
    @media(max-width:340px){.asw-field-row{grid-template-columns:1fr}}
    .asw-flabel{display:flex;align-items:center;gap:var(--sp-1);font-size:var(--t-xs);font-weight:var(--w-normal);color:var(--ink-2);margin-bottom:var(--sp-2);letter-spacing:0}
    .asw-flabel b{color:var(--ink-3);font-weight:var(--w-normal)}
    .asw-form input,.asw-form textarea{width:100%;font-family:inherit;font-size:var(--t-md);color:var(--ink);padding:var(--sp-2) 0;border-radius:0;border:none;border-bottom:1px solid var(--rule-2);background:none;outline:none;transition:border-color var(--ms) var(--ease)}
    .asw-form textarea{resize:vertical;min-height:56px;max-height:150px;line-height:1.8}
    .asw-form input::placeholder,.asw-form textarea::placeholder{color:var(--ink-3)}
    .asw-form input:focus,.asw-form textarea:focus{border-bottom-color:var(--ink);box-shadow:none}
    .asw-field.invalid input,.asw-field.invalid textarea{border-bottom-color:var(--asw-error);box-shadow:none}
    .asw-field.ok input{border-bottom-color:var(--rule-2)}
    .asw-ferr{display:none;font-size:var(--t-xs);font-weight:var(--w-normal);color:var(--asw-error);margin-top:var(--sp-2)}
    .asw-field.invalid .asw-ferr{display:block}

    .asw-form-submit{width:100%;display:inline-flex;align-items:center;justify-content:center;gap:var(--sp-2);margin-top:var(--sp-4);height:44px;padding:0 var(--sp-6);border:1px solid var(--ink);border-radius:var(--rad);background:var(--ink);color:var(--paper);font-family:inherit;font-size:var(--t-md);font-weight:var(--w-medium);cursor:pointer;box-shadow:none;transition:background var(--ms) var(--ease),color var(--ms) var(--ease),opacity var(--ms) var(--ease)}
    .asw-form-submit:hover{background:var(--paper);color:var(--ink)}
    .asw-form-submit:active{opacity:.7}
    .asw-form-submit:disabled{background:none;border-color:var(--rule);color:var(--ink-3);cursor:default}
    .asw-form-submit svg{width:14px;height:14px}
    .asw-spinner{width:14px;height:14px;flex:none;border:1.5px solid color-mix(in srgb,currentColor 28%,transparent);border-top-color:currentColor;border-radius:var(--rad-pill);animation:m-spin .7s linear infinite}
    .asw-form-note{display:flex;align-items:center;justify-content:center;gap:var(--sp-1);font-size:10.5px;color:var(--ink-3);text-align:center;margin-top:var(--sp-4);line-height:1.7}
    .asw-form-note svg{width:11px;height:11px;flex:none}
    .asw-form-err{display:none;font-size:var(--t-xs);font-weight:var(--w-normal);color:var(--asw-error);margin-top:var(--sp-3);text-align:center}
    .asw-form-err.show{display:block}

    .asw-form-success{display:flex;flex-direction:column;align-items:center;text-align:center;gap:var(--sp-3);padding:var(--sp-8) var(--sp-2) var(--sp-4);animation:m-fade var(--mm) var(--ease-out) both}
    .asw-checkmark{width:44px;height:44px;stroke:var(--ink);stroke-width:1.5;fill:none;stroke-linecap:round;stroke-linejoin:round}
    .asw-checkmark circle{stroke:var(--rule-2);stroke-dasharray:170;stroke-dashoffset:170;animation:m-draw 600ms var(--ease-out) 60ms forwards}
    .asw-checkmark path{stroke-dasharray:44;stroke-dashoffset:44;animation:m-draw 360ms var(--ease-out) 540ms forwards}
    .asw-fs-title{font-size:var(--t-lg);font-weight:var(--w-medium);color:var(--ink);margin-top:var(--sp-2);letter-spacing:-.018em}
    .asw-fs-desc{font-size:var(--t-sm);color:var(--ink-2);line-height:1.9;max-width:260px}
    .asw-fs-close{margin-top:var(--sp-4);padding:0;border:none;background:none;color:var(--ink-2);font-family:inherit;font-size:var(--t-sm);font-weight:var(--w-normal);cursor:pointer;text-decoration:underline;text-underline-offset:3px;text-decoration-color:var(--rule-2);transition:color var(--ms) var(--ease)}
    .asw-fs-close:hover{color:var(--ink);text-decoration-color:currentColor}
    .asw-leadform-wrap{animation:m-fade var(--mm) var(--ease-out) both}
    .asw-leadform{position:relative;flex:1 1 100%;margin:var(--sp-2) 0;padding:var(--sp-4) 0 0;border:none;border-top:1px solid var(--rule);border-radius:0;background:none;animation:m-fade var(--mm) var(--ease-out) both}

    /* ───────────────────────────────────────────────────────────────────
       MOTION — opacity and small translation only
       ─────────────────────────────────────────────────────────────────── */
    @keyframes m-fade{from{opacity:0}to{opacity:1}}
    @keyframes m-rise{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
    @keyframes m-spin{to{transform:rotate(360deg)}}
    @keyframes m-bounce{0%,100%{opacity:.25}50%{opacity:1}}
    ...
    @media(max-width:480px){
      .asw-fab{width:48px;height:48px}
      .asw-panel{width:min(var(--asw-panel-w),calc(100vw - var(--sp-6)));height:min(var(--asw-panel-h),calc(100dvh - 84px))}
      .asw-root.mobile-fullscreen .asw-panel{position:fixed!important;inset:0!important;width:100vw!important;max-width:none!important;height:100vh!important;height:100dvh!important;max-height:none!important;border-radius:0!important;border:none!important;box-shadow:none!important;transform:translateY(12px)!important}
      ...
      .asw-messages{padding:var(--sp-6) var(--sp-5) var(--sp-4)}
      .asw-header,.asw-footer{padding-inline:var(--sp-5)}
      .asw-row{max-width:94%}
    }
    @media(prefers-reduced-motion:reduce){
      .asw-panel,.asw-fab,.asw-suggestion,.asw-teaser,.asw-scrollbtn,.asw-toast,.asw-sheet,.asw-sheet-card,.asw-chan,.asw-send,.asw-fab-label{transition:none}
      .asw-typing-dot,.asw-wave span,.asw-spinner{animation:none}
    ...
    .asw :focus-visible{outline:1px solid var(--ink);outline-offset:2px}
    .asw ::selection{background:var(--g-3)}
